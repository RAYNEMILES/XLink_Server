...
#get the ports for server demo from config.yaml
service_port_name=openImDemoPort
...
while [ $check -ge 1 ]; do
    ...
    kill -s 9 $oldPid
    echo "kill" ${demo_server_name} " oldPid" $oldPid >>../logs/openIM.log 2>&1
    check=$(ps aux | grep -w ./${demo_server_name} | grep -v grep | wc -l)
done
...
for ((i = 0; i < ${#api_ports[@]}; i++)); do
    nohup ./${demo_server_name} -port ${api_ports[$i]}   >>../logs/openIM.log.${day} 2>&1 &
done
