...
func MsgToUser(pushMsg *pbPush.PushMsgReq) {
	...
	log.NewError(pushMsg.OperationID, "Get msg from msg_transfer And push msg", pushMsg.String())
	if len(grpcCons) == 0 {
	...
	if isOfflinePush && pushMsg.PushToUserID != pushMsg.MsgData.SendID {
		...
		if callbackResp.ActionCode != constant.ActionAllow {
			log.NewError(pushMsg.OperationID, utils.GetSelfFuncName(), "offlinePush stop")
			return
		}
		if offlinePusher == nil {
			log.Error("Sandman Push S2.1", pushMsg.MsgData)
			return
		}
		...
		log.Error("Sandman Push S3", pushResult, pushMsg.MsgData)

		if err != nil {
			log.NewError(pushMsg.OperationID, "offline push error", pushMsg.String(), err.Error())
		} else {
			log.Error(pushMsg.OperationID, "offline push return result is ", pushResult, pushMsg.MsgData)
		}
	}
}
...
func MsgToSuperGroupUser(pushMsg *pbPush.PushMsgReq) {
	var wsResult []*pbRelay.SingelMsgToUserResultList
	isOfflinePush := utils.GetSwitchFromOptions(pushMsg.MsgData.Options, constant.IsOfflinePush)
	log.NewError(pushMsg.OperationID, "Get super group msg from msg_transfer And push msg", pushMsg.String())
	...
	if isOfflinePush {
		...
		totalItraCount := len(onlineFailedUserIDList) / 1000
		for i := 0; i <= totalItraCount; i++ {
			startIndex := i * 1000
			endIndex := startIndex + 1000
			if i == totalItraCount {
				endIndex = startIndex - 1 + (len(onlineFailedUserIDList) % 1000)
			}
			subArrayOfUsers := getSubArray(onlineFailedUserIDList, startIndex, endIndex)
			pushResult, err := offlinePusher.Push(subArrayOfUsers, content, jsonCustomContent, pushMsg.OperationID, opts)
			log.Error("Sandman Push 3", pushResult, pushMsg.MsgData)
			if err != nil {
				log.Error(pushMsg.OperationID, "offline push error", pushMsg.String(), err.Error())
			} else {
				log.Error(pushMsg.OperationID, "offline push return result is ", pushResult, pushMsg.MsgData)
			}
		}
...
func getSubArray(arr []string, startIndex int, endIndex int) []string {
