...
func MinioUploadFile(c *gin.Context) {
	...
	defer func() {
		if r := recover(); r != nil {
			...
			log.NewError("", "MinioUploadFile panic", "panic:", string(buf))
	...
	if err != nil {
		...
		c.JSON(http.StatusBadRequest, gin.H{"errCode": 400, "errMsg": "upload file error" + err.Error()})
		return
	}
	if req.FileType == constant.ImageType {
		...
		if err == nil {
			log.Debug(req.OperationID, utils.GetSelfFuncName(), "Uploaded Image width And Height ", decodeConfig.Width, decodeConfig.Height)
...
func MinioUploadPersistentFile(c *gin.Context) {
	var (
		req  apiStruct.MinioUploadFileReq
		resp apiStruct.MinioUploadFileResp
	)
	defer func() {
		if r := recover(); r != nil {
			...
			buf := make([]byte, 1<<16)
			runtime.Stack(buf, true)
			log.NewError("", "MinioUploadPersistentFile panic", "panic:", string(buf))
			c.JSON(http.StatusBadRequest, gin.H{"errCode": 400, "errMsg": "missing file or snapShot args"})
			return
		}
	}()
	...
	case constant.VideoType:
		...
		snapShotNewName, snapShotNewType := utils.GetNewFileNameAndContentType(snapShotFile.Filename, constant.ImageType)
		log.Debug(req.OperationID, utils.GetSelfFuncName(), snapShotNewName, snapShotNewType)
		_, err = MinioClient.PutObject(context.Background(), config.Config.Credential.Minio.PersistenceBucket, snapShotNewName, snapShotFileObj, snapShotFile.Size, minio.PutObjectOptions{ContentType: snapShotNewType})
		if err != nil {
			log.NewError(req.OperationID, utils.GetSelfFuncName(), "PutObject snapShotFile error", err.Error())
			c.JSON(http.StatusBadRequest, gin.H{"errCode": 400, "errMsg": err.Error()})
			return
		}
		resp.SnapshotURL = config.Config.Credential.Minio.Endpoint + "/" + config.Config.Credential.Minio.PersistenceBucket + "/" + snapShotNewName
		resp.SnapshotNewName = snapShotNewName
	}
	file, err := c.FormFile("file")
	if err != nil {
	...
	newName, newType := utils.GetNewFileNameAndContentType(file.Filename, req.FileType)
	log.Debug(req.OperationID, utils.GetSelfFuncName(), config.Config.Credential.Minio.PersistenceBucket, newName, fileObj, file.Size, newType, MinioClient.EndpointURL())
	_, err = MinioClient.PutObject(context.Background(), config.Config.Credential.Minio.PersistenceBucket, newName, fileObj, file.Size, minio.PutObjectOptions{ContentType: newType})
	if err != nil {
		log.NewError(req.OperationID, utils.GetSelfFuncName(), "upload file error", err.Error())
		c.JSON(http.StatusBadRequest, gin.H{"errCode": 400, "errMsg": "upload file error" + err.Error()})
		return
	...
	resp.NewName = newName
	resp.URL = config.Config.Credential.Minio.Endpoint + "/" + config.Config.Credential.Minio.PersistenceBucket + "/" + newName
	log.NewInfo(req.OperationID, utils.GetSelfFuncName(), "resp: ", resp)
	c.JSON(http.StatusOK, gin.H{"errCode": 0, "errMsg": "", "data": resp})
	return
}
...
func UploadUpdateApp(c *gin.Context) {
	...
	if err != nil {
		...
		c.JSON(http.StatusBadRequest, gin.H{"errCode": 400, "errMsg": "PutObject file error" + err.Error()})
		return
	}
	if yamlName != "" {
		...
		if err == nil {
			...
			if err != nil {
				...
				c.JSON(http.StatusBadRequest, gin.H{"errCode": 400, "errMsg": "PutObject yaml error" + err.Error()})
				return
			}
		} else {
