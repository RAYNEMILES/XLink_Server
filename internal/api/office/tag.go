...
func GetUserTags(c *gin.Context) {
	...
	if err != nil {
		...
		c.JSON(http.StatusBadRequest, gin.H{"errCode": 400, "errMsg": "GetUserTags rpc server failed" + err.Error()})
		return
	}
...
func CreateTag(c *gin.Context) {
	...
	if err != nil {
		log.NewError(req.OperationID, utils.GetSelfFuncName(), "GetUserTags failed", err.Error())
		c.JSON(http.StatusBadRequest, gin.H{"errCode": 400, "errMsg": "CreateTag rpc server failed" + err.Error()})
		return
	}
	if err := utils.CopyStructFields(&resp.CommResp, respPb.CommonResp); err != nil {
		log.NewError(req.OperationID, utils.GetSelfFuncName(), "CopyStructFields failed", err.Error())
...
func GetTagSendLogs(c *gin.Context) {
	...
	if err != nil {
		...
		c.JSON(http.StatusBadRequest, gin.H{"errCode": 400, "errMsg": "GetTagSendLogs rpc server failed" + err.Error()})
		return
	}
	if err := utils.CopyStructFields(&resp.CommResp, respPb.CommonResp); err != nil {
		log.NewDebug(req.OperationID, utils.GetSelfFuncName(), "CopyStructFields failed", err.Error())
