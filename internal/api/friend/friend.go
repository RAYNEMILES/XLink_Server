...
func AddBlack(c *gin.Context) {
	...
	if existed {
		req.CommID.OpUserID = userIDInter.(string)
	}
...
func ImportFriend(c *gin.Context) {
	...
	if err != nil {
		...
		c.JSON(http.StatusBadRequest, gin.H{"errCode": 400, "errMsg": "ImportFriend failed "})
		return
	}
...
func AddFriend(c *gin.Context) {
	...
	recentFriendID, _ := db.DB.GetTimeLimitOnActionType(db.FriendRequestCacheType+params.FromUserID+params.ToUserID, token)
	if recentFriendID != "" {
		log.NewError("User have recently added friend not more then 10 sec")
		c.JSON(http.StatusBadRequest, gin.H{"errCode": constant.ErrAddFriendStoped.ErrCode, "errMsg": constant.ErrAddFriendStoped.ErrMsg})
		return
	}
	...
	if err != nil {
		...
		c.JSON(http.StatusBadRequest, gin.H{"errCode": 400, "errMsg": "call AddFriend rpc server failed"})
		return
	}

	if RpcResp.CommonResp.ErrCode == constant.OK.ErrCode {
		if err := db.DB.SetTimeLimitOnActionType(db.FriendRequestCacheType+params.FromUserID+params.ToUserID, token, params.ToUserID); err != nil && RpcResp.CommonResp != nil {
			log.NewError(params.OperationID, utils.GetSelfFuncName(), "cache failed, Insertion new add friend failed", err.Error())
		}
	}
...
func AddFriendResponse(c *gin.Context) {
	...
	if err != nil {
		...
		c.JSON(http.StatusBadRequest, gin.H{"errCode": 400, "errMsg": "call add_friend_response rpc server failed"})
		return
	}
...
func DeleteFriend(c *gin.Context) {
	...
	if err != nil {
		...
		c.JSON(http.StatusBadRequest, gin.H{"errCode": 400, "errMsg": "call delete_friend rpc server failed"})
		return
	}
...
func GetBlacklist(c *gin.Context) {
	...
	if err != nil {
		...
		c.JSON(http.StatusBadRequest, gin.H{"errCode": 400, "errMsg": "call get blacklist rpc server failed"})
		return
	}
	...
	for _, v := range RpcResp.BlackUserInfoList {
		log.Debug("", "rpc v.Gender: ", v.Gender)
	...
	for index, v := range RpcResp.BlackUserInfoList {
		log.Debug("", "v.Gender: ", v.Gender)
		resp.Data[index]["gender"] = strconv.Itoa(int(v.Gender))
	}
...
func SetFriendRemark(c *gin.Context) {
	...
	if err != nil {
		...
		c.JSON(http.StatusBadRequest, gin.H{"errCode": 400, "errMsg": "call set friend comment rpc server failed"})
		return
	}
...
func GetFriendRemarkOrNick(c *gin.Context) {
	params := api.GetFriendRemarkOrNickReq{}
	if err := c.BindJSON(&params); err != nil {
		// log.NewError("0", "BindJSON failed ", err.Error())
		// c.JSON(http.StatusBadRequest, gin.H{"errCode": 400, "errMsg": err.Error()})
	...
	req := &rpc.GetFriendRemarkOrNickReq{}
	req.ForUserID = params.ForUserID
	req.GroupID = params.GroupID
	...
	log.NewInfo(req.OperationID, "SetFriendComment args ", req.String())
	...
	client := rpc.NewFriendClient(etcdConn)
	RpcResp, err := client.GetFriendRemarkOrNick(context.Background(), req)
	if err != nil {
		log.NewError(req.OperationID, "SetFriendComment failed ", err.Error(), req.String())
		c.JSON(http.StatusBadRequest, gin.H{"errCode": 400, "errMsg": "call set friend comment rpc server failed"})
		return
	}
	resp := api.GetFriendRemarkOrNickResp{}
	resp.GetFriendRemarkOrNick.RemarkNickName = RpcResp.RemarkNickName

	// resposeInterface := make(map[string]string)
	// resposeInterface["remarkNickName"] = RpcResp.RemarkNickName

	// log.NewInfo(req.OperationID, "Sandgirl -> SetFriendComment api return ", resposeInterface)
	c.JSON(http.StatusOK, resp)
}
...
func RemoveBlack(c *gin.Context) {
	...
	if err != nil {
		...
		c.JSON(http.StatusBadRequest, gin.H{"errCode": 400, "errMsg": "call remove blacklist rpc server failed"})
		return
	}
...
func IsFriend(c *gin.Context) {
	...
	if err != nil {
		...
		c.JSON(http.StatusBadRequest, gin.H{"errCode": 400, "errMsg": "call add friend rpc server failed"})
		return
	}
...
func GetFriendsInfo(c *gin.Context) {
	...
	req := &rpc.GetFriendsInfoReq{OperationID: params.OperationID, FriendUserIDs: params.FriendUserIDs}
	...
	log.NewInfo(req.OperationID, "GetFriendInfo args ", req.String())
	...
	if err != nil {
		log.NewError(req.OperationID, "GetFriendsInfo failed ", err.Error(), req.String())
		c.JSON(http.StatusBadRequest, gin.H{"errCode": 400, "errMsg": "call get friends info rpc server failed"})
		return
	}

	resp := api.GetFriendsInfoResp{CommResp: api.CommResp{ErrCode: RpcResp.ErrCode, ErrMsg: RpcResp.ErrMsg}, FriendInfoList: RpcResp.FriendInfoList}
	resp.Data = jsonData.JsonDataList(resp.FriendInfoList)

	//fix the createTime string problem
	//friendList := []map[string]interface{}{}
	//for _, info := range resp.Data {
	//	if friendUser, ok := info["friendUser"]; ok {
	//		newFriendUser := friendUser.(map[string]interface{})
	//		createTime := newFriendUser["createTime"]
	//		s, _ := createTime.(string)
	//		newFriendUser["createTime"], err = strconv.ParseInt(s, 10, 64)
	//		info["friendUser"] = newFriendUser
	//		friendList = append(friendList, info)
	//	}
	//}
	//resp.Data = friendList

	log.NewInfo(req.OperationID, "GetFriendInfo api return ", resp)
	c.JSON(http.StatusOK, resp)
}
...
func GetFriendList(c *gin.Context) {
	...
	if err != nil {
		...
		c.JSON(http.StatusBadRequest, gin.H{"errCode": 400, "errMsg": "call get friend list rpc server failed"})
		return
	}
	...
	//friendList := []map[string]interface{}{}
	//for _, info := range resp.Data {
	//	log.NewInfo(req.CommID.OperationID, utils.GetSelfFuncName(), "info-1:", info)
	//	friendUser := info["friendUser"].(map[string]interface{})
	//	createTime := friendUser["createTime"]
	//	s, _ := createTime.(uint32)
	//	friendUser["createTime"] = int32(s)
	//	info["friendUser"] = friendUser
	//	log.NewInfo(req.CommID.OperationID, utils.GetSelfFuncName(), "info-2:", info)
	//	friendList = append(friendList, info)
	//}
	//resp.Data = friendList
...
func GetFriendApplyList(c *gin.Context) {
	...
	if err != nil {
		...
		c.JSON(http.StatusBadRequest, gin.H{"errCode": 400, "errMsg": "call get friend apply list rpc server failed"})
		return
	}
...
func GetSelfFriendApplyList(c *gin.Context) {
	...
	if err != nil {
		...
		c.JSON(http.StatusBadRequest, gin.H{"errCode": 400, "errMsg": "call get self apply list rpc server failed"})
		return
	}
...
func AddBlackFriends(c *gin.Context) {
	params := api.AddBlackFriendsReq{}
	if err := c.BindJSON(&params); err != nil {
		log.NewError("0", "BindJSON failed ", err.Error())
		c.JSON(http.StatusBadRequest, gin.H{"errCode": 400, "errMsg": err.Error()})
		return
	}
	req := &rpc.AddBlackFriendsReq{}
	err := utils.CopyStructFields(&req, params)
	if err != nil {
		log.NewError("0", "Copy struct failed ", err.Error())
		c.JSON(http.StatusBadRequest, gin.H{"errCode": 400, "errMsg": err.Error()})
		return
	}

	log.NewInfo(params.OperationID, "AddBlackFriends args ", req.String())
	...
	if userID != req.FromUserID {
		log.NewError(params.OperationID, utils.GetSelfFuncName(), "user is banned!", req.OpUserID)
		c.JSON(http.StatusForbidden, gin.H{"errCode": "403", "errMsg": "user is banned"})
		return
	}

	etcdConn := getcdv3.GetConn(config.Config.Etcd.EtcdSchema, strings.Join(config.Config.Etcd.EtcdAddr, ","), config.Config.RpcRegisterName.OpenImFriendName, req.OperationID)
	client := rpc.NewFriendClient(etcdConn)
	rpcResp, err := client.AddBlackFriends(context.Background(), req)
	if err != nil {
		log.NewError(req.OperationID, "AddBlackFriends failed ", err.Error())
		c.JSON(http.StatusBadRequest, gin.H{"errCode": 400, "errMsg": "call add black friend rpc server failed"})
		return
	}

	resp := &api.AddBlacklistResp{CommResp: api.CommResp{ErrCode: rpcResp.CommonResp.ErrCode, ErrMsg: rpcResp.CommonResp.ErrMsg}}
	log.NewInfo(req.OperationID, "AddBlackFriends api return ", resp)
	c.JSON(http.StatusOK, resp)

}

func GetBlackFriends(c *gin.Context) {
	params := &api.GetBlackFriendsReq{}
	if err := c.BindJSON(&params); err != nil {
		log.NewError("0", "BindJSON failed ", err.Error())
		c.JSON(http.StatusBadRequest, gin.H{"errCode": 400, "errMsg": err.Error()})
		return
	...
	log.NewInfo(params.OperationID, "GetBlackFriends args ", req.String())
	...
	client := rpc.NewFriendClient(etcdConn)
	rpcResp, err := client.GetBlackFriends(context.Background(), req)
	if err != nil {
		log.NewError(req.OperationID, "GetBlackFriends failed ", err.Error(), req.String())
		c.JSON(http.StatusBadRequest, gin.H{"errCode": 400, "errMsg": "call get blacklist rpc server failed"})
		return
	}

	resp := &api.GetBlackFriendsResp{CommResp: api.CommResp{ErrCode: rpcResp.CommonResp.ErrCode, ErrMsg: rpcResp.CommonResp.ErrMsg}}

	log.Debug("", "black list len: ", len(rpcResp.BlackList))
	err = utils.CopyStructFields(&resp.Data, rpcResp.BlackList)
	if err != nil {
		log.NewError(req.OperationID, "GetBlackFriends failed ", err.Error(), req.String())
		c.JSON(http.StatusBadRequest, gin.H{"errCode": 400, "errMsg": "call get blacklist rpc server failed"})
		return
	...
	log.NewInfo(req.OperationID, "GetBlackFriends api return ", resp)
	c.JSON(http.StatusOK, resp)

}

func RemoveBlackFriends(c *gin.Context) {
	params := api.RemoveBlackFriendsReq{}
	if err := c.BindJSON(&params); err != nil {
		log.NewError("0", "BindJSON failed ", err.Error())
		c.JSON(http.StatusBadRequest, gin.H{"errCode": 400, "errMsg": err.Error()})
		return
	...
	req := &rpc.RemoveBlackFriendsReq{}
	err := utils.CopyStructFields(req, &params)
	if err != nil {
		log.NewError("0", "CopyStructFields failed ", err.Error())
		c.JSON(http.StatusBadRequest, gin.H{"errCode": 400, "errMsg": err.Error()})
		return
	}
	...
	client := rpc.NewFriendClient(etcdConn)
	rpcResp, err := client.RemoveBlackFriends(context.Background(), req)
	if err != nil {
		log.NewError(req.OperationID, "RemoveBlacklist failed ", err.Error(), req.String())
		c.JSON(http.StatusBadRequest, gin.H{"errCode": 400, "errMsg": "call remove blacklist rpc server failed"})
		return
	}
	resp := api.RemoveBlackFriendsResp{CommResp: api.CommResp{ErrCode: rpcResp.CommonResp.ErrCode, ErrMsg: rpcResp.CommonResp.ErrMsg}}
	log.NewInfo(req.OperationID, "RemoveBlackFriends api return ", resp)
	c.JSON(http.StatusOK, resp)
