...
type MsgChannelValue struct {
	chatType      int
	aggregationID string //maybe userID or groupID
	triggerID     string
	...
	for {
		...
		case cmd := <-och.chArrays[channelID]:
			...
			case AggregationMessages:
				...
				//	return
				//}
				//log.NewError(triggerID, utils.GetSelfFuncName(), "msgChannel", msgChannelValue.aggregationID, msgChannelValue.chatType, storageMsgList, "msg storage length", len(storageMsgList), "push length", len(notStoragePushMsgList))
				err, lastSeq := saveUserChatList(msgChannelValue.aggregationID, msgChannelValue.chatType, storageMsgList, triggerID)
				if err != nil {
					...
					och.SendMessageToMongoCH(msgChannelValue.aggregationID, msgChannelValue.chatType, triggerID, storageMsgList, lastSeq)

					//only push private chat messages and group chat messages
					if msgChannelValue.chatType == constant.SingleChatType || msgChannelValue.chatType == constant.GroupChatType {
...
func (och *OnlineHistoryRedisConsumerHandler) SendMessageToMongoCH(aggregationID string, chatType int, triggerID string, messages []*pbMsg.MsgDataToMQ, lastSeq uint64) {
	pid, offset, err := producerToMongo.SendMessage(&pbMsg.MsgDataToMongoByMQ{LastSeq: lastSeq, AggregationID: aggregationID, MessageList: messages, TriggerID: triggerID, ChatType: uint64(chatType)}, aggregationID, triggerID)
	log.NewError("", utils.GetSelfFuncName(), aggregationID, triggerID, messages, lastSeq)
	if err != nil {
			...
			case ConsumerMsgs:
				...
				for aggregationID, v := range aggregationMsgs {
					if len(v) >= 0 {
						...
						//log.Debug(triggerID, "generate channelID", hashCode, channelID, aggregationID)
						//go func(cID uint32, userID string, messages []*pbMsg.MsgDataToMQ) {
						if strings.Contains(aggregationID, "group-") {
							//group chat messages
							aggregationID = strings.TrimPrefix(aggregationID, "group-")
							och.chArrays[channelID] <- Cmd2Value{Cmd: AggregationMessages, Value: MsgChannelValue{chatType: constant.GroupChatType, aggregationID: aggregationID, msgList: v, triggerID: triggerID}}
						} else {
							//private chat messages
							och.chArrays[channelID] <- Cmd2Value{Cmd: AggregationMessages, Value: MsgChannelValue{chatType: constant.SingleChatType, aggregationID: aggregationID, msgList: v, triggerID: triggerID}}
						}
						//}(channelID, userID, v)
					}
				}
...
func (mc *OnlineHistoryRedisConsumerHandler) handleChatWs2Mongo(cMsg *sarama.ConsumerMessage, msgKey string, sess sarama.ConsumerGroupSession) {
	...
	case constant.SingleChatType:
		//log.NewError(msgFromMQ.OperationID, "msg_transfer msg type = SingleChatType", isHistory, isPersist)
		if isHistory {
			err := saveUserChat(msgKey, int(msgFromMQ.MsgData.SessionType), &msgFromMQ)
			if err != nil {
	...
	case constant.GroupChatType:
		//log.NewError(msgFromMQ.OperationID, "msg_transfer msg type = GroupChatType", isHistory, isPersist)
		if isHistory {
			err := saveUserChat(msgFromMQ.MsgData.RecvID, int(msgFromMQ.MsgData.SessionType), &msgFromMQ)
			if err != nil {
	...
	case constant.NotificationChatType:
		//log.NewError(msgFromMQ.OperationID, "msg_transfer msg type = NotificationChatType", isHistory, isPersist)
		if isHistory {
			err := saveUserChat(msgKey, constant.SingleChatType, &msgFromMQ)
			if err != nil {
				log.NewError(operationID, "single data insert to mongo err", err.Error(), msgFromMQ.String())
				return
			}
...
func (och *OnlineHistoryRedisConsumerHandler) ConsumeClaim(sess sarama.ConsumerGroupSession,
	...
	for msg := range claim.Messages() {
		...
		//log.NewError("", utils.GetSelfFuncName(), "online kafka get info to mongo", "msgTopic", msg.Topic, "msgPartition", msg.Partition, "online", msg.Offset, claim.HighWaterMarkOffset())
...
func sendMessageToPush(message *pbMsg.MsgDataToMQ, pushToUserID string) {
	log.NewError(message.OperationID, utils.GetSelfFuncName(), pushToUserID, "msg_transfer send message to push", "message", message.String())
