...
// insert messages from producer to mongodb
func (mc *OnlineHistoryMongoConsumerHandler) handleChatWs2Mongo(cMsg *sarama.ConsumerMessage, msgKey string, _ sarama.ConsumerGroupSession) {
	...
	err = db.DB.BatchInsertChat2DB(msgFromMQ.AggregationID, msgFromMQ.MessageList, int(msgFromMQ.ChatType), msgFromMQ.TriggerID, msgFromMQ.LastSeq)
	if err != nil {
	...
	for _, v := range msgFromMQ.MessageList {
		if v.MsgData.ContentType == constant.DeleteMessageNotification {
			...
			if unexistSeqList, err := db.DB.DelMsgBySeqList(DeleteMessageTips.UserID, int(msgFromMQ.ChatType), DeleteMessageTips.SeqList, v.OperationID); err != nil {
...
func (och *OnlineHistoryMongoConsumerHandler) ConsumeClaim(sess sarama.ConsumerGroupSession,
	...
	for msg := range claim.Messages() {
		//log.NewError("", utils.GetSelfFuncName(), "kafka get info to mongo", "msgTopic", msg.Topic, "msgPartition", msg.Partition, "msg", string(msg.Value), "key", string(msg.Key))
		if len(msg.Value) != 0 {
