...
func (s *statisticsServer) Run() {
	log.NewPrivateLog(constant.OpenImStatisticsLog)
...
func (s *statisticsServer) GetActiveGroup(_ context.Context, req *pbStatistics.GetActiveGroupReq) (*pbStatistics.GetActiveGroupResp, error) {
	...
	activeGroups, err := imdb.GetActiveGroups(fromTime, toTime, 20)
	if err != nil {
...
func (s *statisticsServer) GetActiveUser(_ context.Context, req *pbStatistics.GetActiveUserReq) (*pbStatistics.GetActiveUserResp, error) {
	...
	log.NewDebug(req.OperationID, utils.GetSelfFuncName(), "time: ", fromTime, toTime)
	activeUsers, err := imdb.GetActiveUsers(fromTime, toTime, 20)
	if err != nil {
...
func (s *statisticsServer) GetUserStatistics(_ context.Context, req *pbStatistics.GetUserStatisticsReq) (*pbStatistics.GetUserStatisticsResp, error) {
	...
	activeUserNum, err := imdb.GetActiveUserNum(fromTime.Unix(), toTime.Add(time.Hour*24).Unix())
	if err != nil {
		log.NewError(req.OperationID, utils.GetSelfFuncName(), "GetActiveUserNum failed", err.Error())
		return resp, errors.WrapError(constant.ErrDB)
	}
	increaseUserNum, err := imdb.GetIncreaseUserNum(fromTime.Unix(), toTime.Add(time.Hour*24).Unix())
	if err != nil {
	...
	for i, v := range times {
		go func(wg *sync.WaitGroup, index int, v [2]time.Time) {
			defer wg.Done()
			num, err := imdb.GetActiveUserNum(v[0].Unix(), v[1].Unix())
			if err != nil {
				log.NewError(req.OperationID, utils.GetSelfFuncName(), "GetIncreaseGroupNum", v, err.Error())
			}
			...
			num, err = imdb.GetTotalUserNumByDate(v[1].Unix())
			if err != nil {
			...
			num, err = imdb.GetIncreaseUserNum(v[0].Unix(), v[1].Unix())
			if err != nil {
...
func (s *statisticsServer) GetGameStatistics(_ context.Context, req *pbStatistics.GetGameStatisticsReq) (*pbStatistics.GetGameStatisticsResp, error) {
	resp := &pbStatistics.GetGameStatisticsResp{ActiveGameList: []*pbStatistics.GameResponse{}}
	fromTime, toTime, err := ParseTimeFromTo(req.StatisticsReq.From, req.StatisticsReq.To)

	todayPlayedNum, err := imdb.GetTodayPlayedGameNumber()
	if err != nil {
		log.NewError(req.OperationID, utils.GetSelfFuncName(), "GetGameStatistics failed", err.Error())
		return resp, errors.WrapError(constant.ErrArgs)
	}
	cumulativePlayedNum, err := imdb.CumulativePlayedGameNumber()
	if err != nil {
		log.NewError(req.OperationID, utils.GetSelfFuncName(), "CumulativePlayedGameNumber failed", err.Error())
		return resp, errors.WrapError(constant.ErrArgs)
	}

	resp.GamePlayedToday = todayPlayedNum
	resp.CumulativePlayed = cumulativePlayedNum

	times := GetRangeDate(fromTime, toTime)
	resp.GamePlayedNum = make([]*pbStatistics.DateNumList, len(times), len(times))
	...
	for k, v := range times {
		go func(wg *sync.WaitGroup, index int, v [2]time.Time) {
			defer wg.Done()
			playedNum, err := imdb.GetGamePlayed(v[0].Unix(), v[1].Unix())
			if err != nil {
				log.NewError(req.OperationID, utils.GetSelfFuncName(), "GetGamePlayed", v, err.Error())
			}
			resp.GamePlayedNum[index] = &pbStatistics.DateNumList{
				Date: v[0].String(),
				Num:  int32(playedNum),
			}
			log.Debug("", "resp.GamePlayedNum: ", resp.GamePlayedNum)
		}(wg, k, v)
	}
	wg.Wait()
	log.Debug("", "resp.GamePlayedNum: ", resp.GamePlayedNum)

	games, err := imdb.GetActiveGames()
	if err != nil {
		log.NewError(req.OperationID, utils.GetSelfFuncName(), "GetActiveGames failed", err.Error())
		return resp, errors.WrapError(constant.ErrArgs)
	}

	resp.ActiveGameList = make([]*pbStatistics.GameResponse, len(games))
	for index, game := range games {
		activeGame := &pbStatistics.GameResponse{}
		activeGame.GameCode = game.GameCode
		activeGame.PlayedNumber = game.PlayCounts
		activeGame.GameName = map[string]string{
			"en": game.GameNameEN,
			"cn": game.GameNameCN,
		}
		resp.ActiveGameList[index] = activeGame
	}
