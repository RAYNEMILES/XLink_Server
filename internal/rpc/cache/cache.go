...
func SyncDB2Cache() error {
	...
	go func() {
		defer wg.Done()
		err = updateAllBlackForMomentListToCache()
	}()
	go func() {
		defer wg.Done()
	...
	for _, userInfo := range userList {
		go func() {
			...
			userInfoPb := &commonPb.UserInfo{
				...
				Birth:          utils.GetTimeStringFromTime(userInfo.Birth),
				Email:          userInfo.Email,
				Ex:             userInfo.Ex,
				CreateTime:     int32(userInfo.CreateTime),
...
func updateAllBlackForMomentListToCache() error {
	//log.NewInfo("0", utils.GetSelfFuncName())
	userList, err := imdb.GetDistinctUsersFromBlackMoments()
	if err != nil {
		return err
	}
	for _, user := range userList {
		blackIDList, err := imdb.GetBlackIDForMomentListByUserID(user.OwnerUserID)
		if err != nil {
			log.NewWarn("", utils.GetSelfFuncName(), err.Error())
			continue
		}
		if len(blackIDList) > 0 {
			if err := db.DB.AddBlackUserForMomentsToCache(user.OwnerUserID, blackIDList...); err != nil {
				log.NewWarn("0", utils.GetSelfFuncName(), err.Error())
			}
		}
	}
...
func (s *cacheServer) ReduceBlackFriendsFromCache(ctx context.Context, req *pbCache.ReduceBlackFriendsFromCacheReq) (resp *pbCache.ReduceBlackFriendsFromCacheResp, err error) {

	//log.NewInfo(req.OperationID, utils.GetSelfFuncName(), "req: ", req.String())
	resp = &pbCache.ReduceBlackFriendsFromCacheResp{CommonResp: &pbCache.CommonResp{}}
	if err := db.DB.ReduceBlackUserFromCache(req.UserID, req.ToUsersID...); err != nil {
		log.NewError(req.OperationID, utils.GetSelfFuncName(), err.Error())
		resp.CommonResp.ErrCode = constant.ErrDB.ErrCode
		resp.CommonResp.ErrMsg = constant.ErrDB.ErrMsg
		return resp, nil
...
func (s *cacheServer) AddBlackFriendsToCache(_ context.Context, req *pbCache.AddBlackFriendsToCacheReq) (resp *pbCache.AddBlackFriendsToCacheResp, err error) {

	//log.NewInfo(req.OperationID, utils.GetSelfFuncName(), "req: ", req.String())
	resp = &pbCache.AddBlackFriendsToCacheResp{CommonResp: &pbCache.CommonResp{}}
	if err := db.DB.AddBlackUserToCache(req.UserID, req.BlackUsersID...); err != nil {
		log.NewError(req.OperationID, utils.GetSelfFuncName(), err.Error())
		resp.CommonResp.ErrCode = constant.ErrDB.ErrCode
		resp.CommonResp.ErrMsg = constant.ErrDB.ErrMsg
		return resp, nil
...
func (s *cacheServer) AddBlackFriendsForMomentsToCache(_ context.Context, req *pbCache.AddBlackFriendsToCacheReq) (resp *pbCache.AddBlackFriendsToCacheResp, err error) {

	//log.NewInfo(req.OperationID, utils.GetSelfFuncName(), "req: ", req.String())
	resp = &pbCache.AddBlackFriendsToCacheResp{CommonResp: &pbCache.CommonResp{}}
	if err := db.DB.AddBlackUserForMomentsToCache(req.UserID, req.BlackUsersID...); err != nil {
		log.NewError(req.OperationID, utils.GetSelfFuncName(), err.Error())
		resp.CommonResp.ErrCode = constant.ErrDB.ErrCode
		resp.CommonResp.ErrMsg = constant.ErrDB.ErrMsg
		return resp, nil
...
func (s *cacheServer) ReduceBlackFriendsForMomentsFromCache(ctx context.Context, req *pbCache.ReduceBlackFriendsFromCacheReq) (resp *pbCache.ReduceBlackFriendsFromCacheResp, err error) {

	//log.NewInfo(req.OperationID, utils.GetSelfFuncName(), "req: ", req.String())
	resp = &pbCache.ReduceBlackFriendsFromCacheResp{CommonResp: &pbCache.CommonResp{}}
	if err := db.DB.ReduceBlackUserForMomentsFromCache(req.UserID, req.ToUsersID...); err != nil {
		log.NewError(req.OperationID, utils.GetSelfFuncName(), err.Error())
		resp.CommonResp.ErrCode = constant.ErrDB.ErrCode
		resp.CommonResp.ErrMsg = constant.ErrDB.ErrMsg
		return resp, nil
...
func (s *cacheServer) SwitchBroadcast(_ context.Context, req *pbCache.SwitchBroadcastReq) (*pbCache.SwitchBroadcastResp, error) {

	resp := &pbCache.SwitchBroadcastResp{CommonResp: &pbCache.CommonResp{}}
	err := db.DB.SetUserBroadcastStatus(req.UserID, req.Status)
	if err != nil {
		resp.CommonResp.ErrCode = constant.ErrDB.ErrCode
		resp.CommonResp.ErrMsg = constant.ErrDB.ErrMsg
		return nil, err
...
func (s *cacheServer) GetBroadcast(_ context.Context, req *pbCache.GetBroadcastReq) (*pbCache.GetBroadcastResp, error) {

	resp := &pbCache.GetBroadcastResp{CommonResp: &pbCache.CommonResp{}}
	status, err := db.DB.GetUserBroadcastStatus(req.UserID)
	if err != nil {
		status = 1
		err = db.DB.SetUserBroadcastStatus(req.UserID, int32(status))
		if err != nil {
			resp.CommonResp.ErrCode = constant.ErrDB.ErrCode
			resp.CommonResp.ErrMsg = constant.ErrDB.ErrMsg
			return nil, err
