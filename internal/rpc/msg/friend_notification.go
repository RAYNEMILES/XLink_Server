...
func getFromToUserNickname(fromUserID, toUserID string) (string, string, error) {
	from, err := imdb.GetUserByUserIDEvenDeleted(fromUserID)
	if err != nil {
	...
	if fromNickName == "" {
		fromNickName = from.UserID
	}
	toNickName := to.Nickname
	if toNickName == "" {
		toNickName = to.UserID
	}
	return fromNickName, toNickName, nil
}
...
func friendNotification(commID *pbFriend.CommID, contentType int32, m proto.Message) {
	...
	case constant.UserNotFriendNotification:
		tips.DefaultTips = cn.UserNotFriendNotification.DefaultTips.Tips
	case constant.UserBlockedYouNotification:
		tips.DefaultTips = cn.UserBlockedYouNotification.DefaultTips.Tips
	default:
...
func FriendAddedNotification(operationID, opUserID, fromUserID, toUserID string) {
	...
	commID = pbFriend.CommID{FromUserID: toUserID, ToUserID: fromUserID, OpUserID: opUserID, OperationID: operationID}
	friendNotification(&commID, constant.FriendAddedNotification, &friendAddedTips)
}
...
func UserNotFriendNotification(operationID, userID string, needNotifiedUserID string) {
	userNotFriendTips := open_im_sdk.UserNotFriendTips{UserID: userID}
	commID := pbFriend.CommID{FromUserID: userID, ToUserID: needNotifiedUserID, OpUserID: userID, OperationID: operationID}
	friendNotification(&commID, constant.UserNotFriendNotification, &userNotFriendTips)
}

func OnMsgUserBlockedYouNotification(operationID, userID string, needNotifiedUserID string) {
	userBlockedYouTips := open_im_sdk.UserBlockedYouTips{UserID: userID}
	commID := pbFriend.CommID{FromUserID: userID, ToUserID: needNotifiedUserID, OpUserID: userID, OperationID: operationID}
	friendNotification(&commID, constant.UserBlockedYouNotification, &userBlockedYouTips)
}
