...
func (rpc *rpcChat) Run() {
	log.NewPrivateLog(constant.OpenImMsgLog)
...
func (rpc *rpcChat) runCh() {
	...
	for {
		...
		case msg := <-rpc.delMsgCh:
			...
			db.DB.DelMsgFromCache(msg.UserID, msg.ChatType, msg.SeqList, msg.OperationID)
			unexistSeqList, err := db.DB.DelMsgBySeqList(msg.UserID, msg.ChatType, msg.SeqList, msg.OperationID)
			if err != nil {
