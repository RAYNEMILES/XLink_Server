...
func userRelationshipVerification(data *pbChat.SendMsgReq) (bool, int32, string) {
	if data.MsgData.SessionType == constant.SingleChatType {
		...
		if err != nil {
			...
			if cacheResp.CommonResp.ErrCode != 0 {
				...
				if utils.IsContain(data.MsgData.SendID, cacheResp.UserIDList) {
					if data.MsgData.ContentType != constant.HasReadReceipt &&
						data.MsgData.ContentType != constant.Typing &&
						data.MsgData.ContentType != constant.MomentNotification {
						OnMsgUserBlockedYouNotification(data.OperationID, data.MsgData.RecvID, data.MsgData.SendID)
					}
		...
		if config.Config.MessageVerify.FriendVerify {
			reqGetFriendIDListFromCache := &cacheRpc.GetFriendIDListFromCacheReq{UserID: data.MsgData.SendID, OperationID: data.OperationID}
			reqGetFriendIDListToCache := &cacheRpc.GetFriendIDListFromCacheReq{UserID: data.MsgData.RecvID, OperationID: data.OperationID}
			etcdConn := getcdv3.GetConn(config.Config.Etcd.EtcdSchema, strings.Join(config.Config.Etcd.EtcdAddr, ","), config.Config.RpcRegisterName.OpenImCacheName, data.OperationID)
			if etcdConn == nil {
				errMsg := data.OperationID + "getcdv3.GetConn == nil"
				log.NewError(data.OperationID, errMsg)
			...
			//userIDlist = append(userIDlist, fmt.Sprint(data.MsgData.SendID))
			//getUserinfoFromCacheReq := &cacheRpc.GetUserInfoFromCacheReq{UserIDList: userIDlist, OperationID: data.OperationID}
			//userInfo, err1 := cacheClient.GetUserInfoFromCache(context.Background(), getUserinfoFromCacheReq)
			//if err1 == nil && userInfo != nil {
			//	if len(userInfo.UserInfoList) > 1 && userInfo.UserInfoList[0].SuperUserStatus == 1 {
			//		return true, 0, ""
			//	}
			//	return true, 0, ""
			//} else {
			//	if err1 != nil {
			//		return false, 6013, err.Error()
			//	}
			//	return false, 603, "User info not found"
			//}
			cacheRespFrom, err := cacheClient.GetFriendIDListFromCache(context.Background(), reqGetFriendIDListFromCache)
			cacheRespTo, errto := cacheClient.GetFriendIDListFromCache(context.Background(), reqGetFriendIDListToCache)
			if err != nil || errto != nil {
				log.NewError(data.OperationID, "GetFriendIDListFromCache rpc call failed ", err.Error())
			} else {
				if cacheRespFrom != nil && cacheRespFrom.CommonResp != nil && cacheRespFrom.CommonResp.ErrCode == 0 {
					if cacheRespTo != nil && cacheRespTo.CommonResp != nil && cacheRespTo.CommonResp.ErrCode == 0 {
						if utils.IsContain(data.MsgData.RecvID, cacheRespFrom.UserIDList) && utils.IsContain(data.MsgData.SendID, cacheRespTo.UserIDList) {
							return true, 0, ""
						}
					}
				...
				if data.MsgData.ContentType != constant.HasReadReceipt &&
					data.MsgData.ContentType != constant.Typing &&
					data.MsgData.ContentType != constant.MomentNotification {
					UserNotFriendNotification(data.OperationID, data.MsgData.RecvID, data.MsgData.SendID)
				}
...
// setting options for different type messages
func (rpc *rpcChat) encapsulateMsgData(msg *sdk_ws.MsgData) {
	...
	case constant.Typing:
		...
		utils.SetSwitchFromOptions(msg.Options, constant.IsUnreadCount, false)
		utils.SetSwitchFromOptions(msg.Options, constant.IsOfflinePush, false)
	case constant.MomentNotification:
		utils.SetSwitchFromOptions(msg.Options, constant.IsHistory, false)
		utils.SetSwitchFromOptions(msg.Options, constant.IsPersistent, false)
		utils.SetSwitchFromOptions(msg.Options, constant.IsSenderSync, false)
		utils.SetSwitchFromOptions(msg.Options, constant.IsConversationUpdate, false)
...
func (rpc *rpcChat) SendMsg(_ context.Context, pb *pbChat.SendMsgReq) (*pbChat.SendMsgResp, error) {
	...
	_ = db.GetCurrentTimestampByMill()
	//log.Info(pb.OperationID, "rpc sendMsg come here ", pb.String())

	//verify friendship for single chat
	if pb.MsgData.ContentType < constant.GroupHasReadReceipt && pb.MsgData.ContentType != constant.HasReadReceipt && pb.MsgData.MsgFrom != constant.SysMsgType {
	...
	//if it has callback, request the callback first, if no allow will not go to next step
	...
	//set the message status to revoked in mysql
	if pb.MsgData.ContentType == constant.Revoke {
		im_mysql_model.SwitchChatLogStatusWithClientMsgID(string(pb.MsgData.Content), constant.MsgStatusRevoked)
	}

	go func() {
		// forward count
		if pb.MsgData.ContentType == constant2.Woom {
			woomContent := make(map[string]string)
			e := json.Unmarshal(pb.MsgData.Content, &woomContent)
			if e == nil {
				_ = im_mysql_model.IncrShortVideoForwardNum(woomContent["fileId"])
			}
		}
	}()
	...
	case constant.SingleChatType:
		//call the callback before sending message
		...
		if isSend && msgToMQSingle.MsgData.SendID != msgToMQSingle.MsgData.RecvID {
	...
	case constant.GroupChatType:
		...
		//get userid list for the members were kicked or quited
		...
		//wg.Wait()
		err1 := rpc.sendMsgToKafka(&msgToMQSingle, "group-"+msgToMQSingle.MsgData.GroupID, constant.OnlineStatus)
		if err1 != nil {
			log.NewError(msgToMQSingle.OperationID, "kafka send msg err:RecvID", msgToMQSingle.MsgData.RecvID, msgToMQSingle.String())
			return returnMsg(&replay, pb, 201, "kafka send msg err", "", 0)
		} else {
			//log.Error(msgToMQSingle.OperationID, "kafka send msg :RecvID", msgToMQSingle.MsgData.RecvID, msgToMQSingle.String())
		}

		go saveGroupMessageStatisticToQueue(pb.MsgData.GroupID, pb.MsgData.SendID, pb.MsgData.SendTime)
...
// send broadcastmessages to kafka
func (rpc *rpcChat) SendBroadcastMsg(_ context.Context, pb *pbChat.SendMsgReq) (*pbChat.SendBroadcastMsgResp, error) {
	replay := pbChat.SendMsgResp{}
	_ = db.GetCurrentTimestampByMill()
	//log.Info(pb.OperationID, "rpc sendMsg come here ", pb.String())

	var receiverIDsResponseMap = make(map[string]*pbChat.SendMsgResp)
	var singleChatReceiverIDs []string
	var groupChatReceiverIDs []string
	singleChatReceiverIDs = append(singleChatReceiverIDs, pb.MsgData.BroadcastRecvIDs...)
	groupChatReceiverIDs = append(groupChatReceiverIDs, pb.MsgData.BroadcastGroupIDs...)

	//log.Error(pb.OperationID, "Sandman Broadcast Receivers SendBroadcastMsg", singleChatReceiverIDs, groupChatReceiverIDs)

	pb.MsgData.BroadcastRecvIDs = make([]string, 0)
	pb.MsgData.BroadcastGroupIDs = make([]string, 0)
	//verify friendship for single chat
	if pb.MsgData.ContentType < constant.GroupHasReadReceipt && pb.MsgData.ContentType != constant.HasReadReceipt {
		flag, errCode, errMsg := userRelationshipVerification(pb)
		log.Info(pb.OperationID, "userRelationshipVerification ", flag)
		if !flag {
			replayBroadcast := pbChat.SendBroadcastMsgResp{}
			return returnBroadcastMsg(&replayBroadcast, receiverIDsResponseMap, errCode, errMsg)
		}
	}
	...
	if callbackResp.ActionCode != constant.ActionAllow {
		...
		//log.NewDebug(pb.OperationID, utils.GetSelfFuncName(), "callbackWordFilter result", "end rpc and return", pb.MsgData)
		replayBroadcast := pbChat.SendBroadcastMsgResp{}
		return returnBroadcastMsg(&replayBroadcast, receiverIDsResponseMap, int32(callbackResp.ErrCode), callbackResp.ErrMsg)
	}

	for _, singleCharReceiverID := range singleChatReceiverIDs {
		pb.MsgData.SessionType = constant.SingleChatType
		pb.MsgData.RecvID = singleCharReceiverID
		pb.MsgData.GroupID = ""

		//setting options for different type messages
		// Setting Msg ServerID
		rpc.encapsulateMsgData(pb.MsgData)
		msgToMQSingle := pbChat.MsgDataToMQ{Token: pb.Token, OperationID: pb.OperationID, MsgData: pb.MsgData}

		conversationIDWithSession := utils.GetConversationIDBySessionType(singleCharReceiverID, constant.SingleChatType)
		//call the callback before sending message
		callbackResp := callbackBeforeSendSingleMsg(pb)
		if callbackResp.ErrCode != 0 {
			log.NewError(pb.OperationID, utils.GetSelfFuncName(), "callbackBeforeSendSingleMsg resp: ", callbackResp)
		...
		if callbackResp.ActionCode != constant.ActionAllow {
			...
			log.NewDebug(pb.OperationID, utils.GetSelfFuncName(), "callbackBeforeSendSingleMsg result", "end rpc and return", callbackResp)
			callbackResp, _ := returnMsg(&replay, pb, int32(callbackResp.ErrCode), callbackResp.ErrMsg, "", 0)
			receiverIDsResponseMap[conversationIDWithSession] = callbackResp
			continue
		}
		...
		if isSend && msgToMQSingle.MsgData.SendID != msgToMQSingle.MsgData.RecvID {
			...
			if err1 != nil {
				log.NewError(msgToMQSingle.OperationID, "kafka send msg err :RecvID", msgToMQSingle.MsgData.RecvID, msgToMQSingle.String(), err1.Error())
				callbackResp, _ := returnMsg(&replay, pb, 201, "kafka send msg err", "", 0)
		...
		callbackRespSent, _ := returnMsg(&replay, pb, 0, "", msgToMQSingle.MsgData.ServerMsgID, msgToMQSingle.MsgData.SendTime)
		receiverIDsResponseMap[singleCharReceiverID] = callbackRespSent
	}

	for _, groupChatReceiverID := range groupChatReceiverIDs {
		pb.MsgData.SessionType = constant.GroupChatType
		pb.MsgData.GroupID = groupChatReceiverID
		pb.MsgData.RecvID = ""
		//setting options for different type messages
		// Setting Msg ServerID
		rpc.encapsulateMsgData(pb.MsgData)
		...
		conversationIDWithSession := utils.GetConversationIDBySessionType(groupChatReceiverID, constant.GroupChatType)
		//call the callback before sending message
		callbackResp := callbackBeforeSendGroupMsg(pb)
		if callbackResp.ErrCode != 0 {
			log.NewError(pb.OperationID, utils.GetSelfFuncName(), "callbackBeforeSendGroupMsg resp:", callbackResp)
		...
		//log.Debug(pb.OperationID, "send msg cost time3 ", db.GetCurrentTimestampByMill()-newTime, pb.MsgData.ClientMsgID)
		callbackRespSent, _ := returnMsg(&replay, pb, 0, "", msgToMQSingle.MsgData.ServerMsgID, msgToMQSingle.MsgData.SendTime)
		receiverIDsResponseMap[conversationIDWithSession] = callbackRespSent
	}
	//jsonStr, _ := json.Marshal(receiverIDsResponseMap)
	//log.NewError(pb.OperationID, utils.GetSelfFuncName(), "Broadcast Send Message Map ", jsonStr)
	replayBroadcast := pbChat.SendBroadcastMsgResp{}
	return returnBroadcastMsg(&replayBroadcast, receiverIDsResponseMap, 0, "")
}

func (rpc *rpcChat) SendMegToUsers(ctx context.Context, req *pbChat.SendMegToUsersReq) (*pbChat.SendMegToUsersResp, error) {
	resp := &pbChat.SendMegToUsersResp{}
	resp.ResultList = []*sdk_ws.UserSendMsgResp{}
	pb := &pbChat.SendMsgReq{}
	pb.MsgData = &sdk_ws.MsgData{}
	utils.CopyStructFields(&pb.MsgData, req.MsgData)
	log.Debug("", "req.RecvIDList len: ", len(req.RecvIDList))
	if req.RecvIDList != nil || req.GroupIDList != nil {
		for _, recvId := range req.RecvIDList {
			pb.MsgData.GroupID = ""
			pb.MsgData.RecvID = recvId
			msg, err := rpc.SendMsg(ctx, pb)
			if err != nil {
				resp.FailedIDList = append(resp.FailedIDList, recvId)
				continue
			}
			log.NewDebug("", "recv user: ", recvId, " success")
			resp.ResultList = append(resp.ResultList, &sdk_ws.UserSendMsgResp{
		...
		for _, groupRecvId := range req.GroupIDList {
			utils.CopyStructFields(pb.MsgData, req.MsgData)
			pb.MsgData.GroupID = groupRecvId
			pb.MsgData.RecvID = ""
			msg, err := rpc.SendMsg(ctx, pb)
			if err != nil {
				resp.FailedIDList = append(resp.FailedIDList, groupRecvId)
				continue
			}
			resp.ResultList = append(resp.ResultList, &sdk_ws.UserSendMsgResp{
				ServerMsgID: msg.ServerMsgID,
...
//func (rpc *rpcChat) SendMsg_old(_ context.Context, pb *pbChat.SendMsgReq) (*pbChat.SendMsgResp, error) {
//	replay := pbChat.SendMsgResp{}
//	newTime := db.GetCurrentTimestampByMill()
//	log.Info(pb.OperationID, "rpc sendMsg come here ", pb.String())
//
//	//if message read enable
...
//	}
//
//	if pb.MsgData.ContentType < constant.GroupHasReadReceipt {
//		flag, errCode, errMsg = userRelationshipVerification(pb)
//		log.Info(pb.OperationID, "userRelationshipVerification ", flag)
//		if !flag {
//			return returnMsg(&replay, pb, errCode, errMsg, "", 0)
...
func (rpc *rpcChat) sendMsgToKafka(m *pbChat.MsgDataToMQ, key string, status string) error {
	...
	case constant.OnlineStatus:
		...
		//log.NewInfo(m.OperationID, "sendMsgToKafka   client msgID ", m.MsgData.ClientMsgID)
		if err != nil {
	...
	case constant.OfflineStatus:
		//pid, offset, err := rpc.onlineProducer.SendMessage(m, key, m.OperationID)
		pid, offset, err := rpc.offlineProducer.SendMessage(m, key, m.OperationID)
		if err != nil {
			log.Error(m.OperationID, "kafka send failed", "send data", m.String(), "pid", pid, "offset", offset, "err", err.Error(), "key", key, status)
		}
		return err
...
func returnBroadcastMsg(replay *pbChat.SendBroadcastMsgResp, receiverResponseMap map[string]*pbChat.SendMsgResp, errCode int32, errMsg string) (*pbChat.SendBroadcastMsgResp, error) {
	replay.ErrCode = errCode
	replay.ErrMsg = errMsg
	replay.ResponseWithReceiverIDs = receiverResponseMap
	return replay, nil
}
...
func modifyMessageByUserMessageReceiveOpt(userID, sourceID string, sessionType int, pb *pbChat.SendMsgReq) bool {
	...
	case constant.ReceiveNotNotifyMessage:
		...
		utils.SetSwitchFromOptions(pb.MsgData.Options, constant.IsOfflinePush, false)
		utils.SetSwitchFromOptions(pb.MsgData.Options, constant.IsUnreadCount, false)
		return true
	}
...
func Notification(n *NotificationMsg) {
	...
	var issynctolocal = false //if sync to local database in open_im_sdk
	...
	case constant.OfficialAccountFollowUnfollowNotification:
		pushSwitch = config.Config.Notification.ConversationOptUpdate.OfflinePush.PushSwitch
		title = config.Config.Notification.ConversationOptUpdate.OfflinePush.Title
		desc = config.Config.Notification.ConversationOptUpdate.OfflinePush.Desc
		ex = config.Config.Notification.ConversationOptUpdate.OfflinePush.Ext
	...
	case constant.SuperGroupUpdateNotification:
		reliabilityLevel = constant.UnreliableNotification
	case constant.UserNotFriendNotification:
		pushSwitch = config.Config.Notification.UserNotFriendNotification.OfflinePush.PushSwitch
		title = config.Config.Notification.UserNotFriendNotification.OfflinePush.Title
		desc = config.Config.Notification.UserNotFriendNotification.OfflinePush.Desc
		ex = config.Config.Notification.UserNotFriendNotification.OfflinePush.Ext
		reliabilityLevel = config.Config.Notification.UserNotFriendNotification.Conversation.ReliabilityLevel
		unReadCount = config.Config.Notification.UserNotFriendNotification.Conversation.UnreadCount
		issynctolocal = true
	case constant.UserBlockedYouNotification:
		pushSwitch = config.Config.Notification.UserBlockedYouNotification.OfflinePush.PushSwitch
		title = config.Config.Notification.UserBlockedYouNotification.OfflinePush.Title
		desc = config.Config.Notification.UserBlockedYouNotification.OfflinePush.Desc
		ex = config.Config.Notification.UserBlockedYouNotification.OfflinePush.Ext
		reliabilityLevel = config.Config.Notification.UserBlockedYouNotification.Conversation.ReliabilityLevel
		unReadCount = config.Config.Notification.UserBlockedYouNotification.Conversation.UnreadCount
		issynctolocal = true
		//case constant.MomentNotification:
		//	reliabilityLevel = constant.ReliableNotificationNoMsg
	}
	//We have to hide the notifications If action performed by Admin CMS Panel
	if n.OpFrom == constant.OpFromAdmin {
		reliabilityLevel = constant.UnreliableNotification
		unReadCount = false
		issynctolocal = false
	}
	...
	utils.SetSwitchFromOptions(msg.Options, constant.IsSyncToLocalDataBase, issynctolocal)
	...
	if err != nil {
		...
		log.NewError(req.OperationID, "Notification SendMsg rpc, ", req.String(), reply.String())
	}
...
func saveGroupMessageStatisticToQueue(groupId, sendUserId string, sendTime int64) {
	db.DB.SaveGroupMessageStatistic(groupId + " " + sendUserId + " " + utils.IntToString(int(sendTime)))
}
