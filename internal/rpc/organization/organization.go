...
func (s *organizationServer) Run() {
	log.NewPrivateLog(constant.OpenImOrganizationLog)
...
func (s *organizationServer) GetSubDepartment(ctx context.Context, req *rpc.GetSubDepartmentReq) (*rpc.GetSubDepartmentResp, error) {
	...
	for _, v := range departmentList {
		...
		err, membernum := imdb.GetDepartmentMemberNum(v1.DepartmentID)
		v1.MemberNum = uint32(membernum)
		if err != nil {
			log.Error(req.OperationID, "GetDepartmentMemberNum failed ", err.Error(), v1.DepartmentID)
			continue
		}
		err, departmentnum := imdb.GetSubDepartmentNum(v1.DepartmentID)
		v1.SubDepartmentNum = uint32(departmentnum)
		if err != nil {
