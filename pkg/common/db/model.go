...
func init() {
	...
	DB.SnowFlake, _ = snowflake.NewNode(0)
	...
	if err := createMongoIndex(mongoClient, privateChat, true, "uid"); err != nil {
		fmt.Println("uid", " index create failed", err.Error())
	}
	if err := createMongoIndex(mongoClient, groupChat, true, "uid"); err != nil {
		fmt.Println("uid", " index create failed", err.Error())
	}
	...
	if err := createMongoIndex(mongoClient, moments, false, "creator_id"); err != nil {
		fmt.Println("moments", "-create_time", "index create failed", err.Error())
	}

	if err := createMongoIndex(mongoClient, moments_like, true, "user_id", "moment_id"); err != nil {
		fmt.Println("moment_id", "-create_time", "index create failed", err.Error())
	}

	if err := createMongoIndex(mongoClient, moments_comments, false, "-moment_id", "user_id"); err != nil {
		fmt.Println("moments_comments", "-create_time", "index create failed", err.Error())
	}

	if err := createMongoIndex(mongoClient, favorites, false, "-favorite_id"); err != nil {
		fmt.Println("favorites", "-create_time", "index create failed", err.Error())
	}

	if err := createMongoIndex(mongoClient, favoriteMedia, true, "-obj_name"); err != nil {
		fmt.Println("favorite media", "-obj_name", "index create failed", err.Error())
	}
