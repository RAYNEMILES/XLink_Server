...
func GetDepartmentMemberNum(departmentID string) (error, int64) {
	dbConn, err := db.DB.MysqlDB.DefaultGormDB()
	if err != nil {
...
func GetSubDepartmentNum(departmentID string) (error, int64) {
	dbConn, err := db.DB.MysqlDB.DefaultGormDB()
	if err != nil {
		return utils.Wrap(err, "DefaultGormDB failed"), 0
	}
...
func SetDepartmentRelatedGroupID(groupID, departmentID string) error {
	...
	return dbConn.Model(&department).Where("department_id=?", departmentID).Updates(department).Error
}
