...
func UpdateGroupRequest(groupRequest db.GroupRequest) error {
	...
	return dbConn.Table("group_requests").Where("group_id=? and user_id=?", groupRequest.GroupID, groupRequest.UserID).Updates(&groupRequest).Error
}
...
func InsertIntoGroupRequest(toInsertInfo db.GroupRequest) error {
	...
	u := dbConn.Table("group_requests").Where("group_id=? and user_id=?", toInsertInfo.GroupID, toInsertInfo.UserID).Updates(&toInsertInfo)
	if u.RowsAffected != 0 {
		return nil
	}
