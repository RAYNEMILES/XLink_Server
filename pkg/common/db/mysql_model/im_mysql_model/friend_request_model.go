...
func InsertFriendApplication(friendRequest *db.FriendRequest, args map[string]interface{}) error {
	...
	//t := dbConn.Table("friend_requests").Where("from_user_id = ? and to_user_id = ?", friendRequest.FromUserID, friendRequest.ToUserID).Select("*").Updates(*friendRequest)
	//if t.RowsAffected == 0 {
	...
	if friendRequest.HandleTime != nil {
		if friendRequest.HandleTime.Unix() <= 0 {
			tmpTime := utils.UnixSecondToTime(0)
