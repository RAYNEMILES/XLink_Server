...
func GetFriendRemarkByUserID(OwnerUserID, FriendUserID string) (string, error) {
	dbConn, err := db.DB.MysqlDB.DefaultGormDB()
	if err != nil {
		return "", err
	}
	var remark string
	err = dbConn.Table("friends").Where("owner_user_id=? and friend_user_id=?", OwnerUserID, FriendUserID).Pluck("remark", &remark).Error
	if err != nil {
		return "", err
	}
	return remark, nil
