...
func (wsRouter *WsFuncRouter) GetFriendApplicationList(input string, operationID string) {
	...
	userWorker.Friend().GetFriendApplicationList(&BaseSuccessFailed{runFuncName(), operationID, wsRouter.uId}, operationID)
}
...
func (wsRouter *WsFuncRouter) GetFriendRemarkOrNick(userIDGroupID string, operationID string) {
	userWorker := open_im_sdk.GetUserWorkerNew(wsRouter.uId)
	if !wsRouter.checkResourceLoadingAndKeysIn(userWorker, userIDGroupID, operationID, runFuncName(), nil) {
		return
	}
	userWorker.Friend().GetFriendRemarkOrNick(&BaseSuccessFailed{runFuncName(), operationID, wsRouter.uId}, userIDGroupID, operationID)
}
func (wsRouter *WsFuncRouter) GetUserRemarkLocal(userIDJson string, operationID string) {
	m := make(map[string]interface{})
	if err := json.Unmarshal([]byte(userIDJson), &m); err != nil {
		log.Info(operationID, utils.GetSelfFuncName(), "unmarshal failed", userIDJson, err.Error())
		wsRouter.GlobalSendMessage(EventData{cleanUpfuncName(runFuncName()), StatusBadParameter, "unmarshal failed", "", operationID})
		return
	}

	userWorker := open_im_sdk.GetUserWorkerNew(wsRouter.uId)
	if !wsRouter.checkResourceLoadingAndKeysIn(userWorker, userIDJson, operationID, runFuncName(), m, "userID") {
		return
	}
	userWorker.Friend().GetUserRemarkLocal(&BaseSuccessFailed{runFuncName(), operationID, wsRouter.uId}, m["userID"].(string), operationID)
}
