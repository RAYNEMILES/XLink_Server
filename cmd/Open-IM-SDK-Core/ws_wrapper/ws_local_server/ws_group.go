...
func (g *GroupCallback) OnGroupMembersSyncedStarted(groupInfo string) {
	SendOneUserMessage(EventData{cleanUpfuncName(runFuncName()), 0, "", groupInfo, "0"}, g.uid)
}

func (g *GroupCallback) OnGroupMembersSyncedFinished(groupInfo string) {
	SendOneUserMessage(EventData{cleanUpfuncName(runFuncName()), 0, "", groupInfo, "0"}, g.uid)
}
...
func (wsRouter *WsFuncRouter) GetGroupMemberList(input, operationID string) { //(groupId string, filter int32, next int32, callback Base) {
	...
	userWorker.Group().GetGroupMemberList(&BaseSuccessFailed{runFuncName(), operationID, wsRouter.uId},
		m["groupID"].(string), int32(m["filter"].(float64)), int32(m["offset"].(float64)), int32(m["count"].(float64)), m["nameSearch"].(string), operationID)
}
...
func (wsRouter *WsFuncRouter) SyncGroupUpdatesVersionByGroupIDV2(input string, operationID string) {
	m := make(map[string]interface{})
	if err := json.Unmarshal([]byte(input), &m); err != nil {
		log.Info(operationID, utils.GetSelfFuncName(), "unmarshal failed", input, err.Error())
		wsRouter.GlobalSendMessage(EventData{cleanUpfuncName(runFuncName()), StatusBadParameter, "unmarshal failed", "", operationID})
	...
	userWorker.Group().SyncGroupUpdatesVersionByGroupIDV2(&BaseSuccessFailed{runFuncName(), operationID, wsRouter.uId},
		m["groupID"].(string), operationID)
}

func (wsRouter *WsFuncRouter) GetGroupMembersSyncStatus(input string, operationID string) {
	m := make(map[string]interface{})
	if err := json.Unmarshal([]byte(input), &m); err != nil {
		log.Info(operationID, utils.GetSelfFuncName(), "unmarshal failed", input, err.Error())
		wsRouter.GlobalSendMessage(EventData{cleanUpfuncName(runFuncName()), StatusBadParameter, "unmarshal failed", "", operationID})
	...
	userWorker.Group().GetGroupMembersSyncStatus(&BaseSuccessFailed{runFuncName(), operationID, wsRouter.uId},
		m["groupID"].(string), operationID)
}

func (wsRouter *WsFuncRouter) GetFriendListNotMemberOfTheGroup(input string, operationID string) {
	m := make(map[string]interface{})
	if err := json.Unmarshal([]byte(input), &m); err != nil {
		log.Info(operationID, utils.GetSelfFuncName(), "unmarshal failed", input, err.Error())
		wsRouter.GlobalSendMessage(EventData{cleanUpfuncName(runFuncName()), StatusBadParameter, "unmarshal failed", "", operationID})
	...
	userWorker.Group().GetFriendListNotMemberOfTheGroup(&BaseSuccessFailed{runFuncName(), operationID, wsRouter.uId},
		m["groupID"].(string), operationID)
}
