...
func (wsRouter *WsFuncRouter) CreateTextMessage(input string, operationID string) {
	...
	if input != "" {
		...
		wsRouter.GlobalSendMessage(EventData{cleanUpfuncName(runFuncName()), constant.ErrArgs.ErrCode, constant.ErrArgs.ErrMsg, "", operationID})
	}

}
func (wsRouter *WsFuncRouter) CreateEmojiMessage(input string, operationID string) {
	userWorker := open_im_sdk.GetUserWorkerNew(wsRouter.uId)
	if !wsRouter.checkResourceLoadingAndKeysIn(userWorker, input, operationID, runFuncName(), nil) {
		return
	}
	...
	input = strings.TrimSpace(input)
	if input != "" {
		msg := userWorker.Conversation().CreateEmojiMessage(input, operationID)
		wsRouter.GlobalSendMessage(EventData{cleanUpfuncName(runFuncName()), 0, "", msg, operationID})
	} else {
		wsRouter.GlobalSendMessage(EventData{cleanUpfuncName(runFuncName()), constant.ErrArgs.ErrCode, constant.ErrArgs.ErrMsg, "", operationID})
	}
...
func (wsRouter *WsFuncRouter) CreateArticleMessage(input string, operationID string) {
	...
	userWorker := open_im_sdk.GetUserWorkerNew(wsRouter.uId)
	if !wsRouter.checkResourceLoadingAndKeysIn(userWorker, input, operationID, runFuncName(), m, "articleID", "articleTitle", "officialID", "officialName", "officialFaceUrl") {
		return
	}
	msg := userWorker.Conversation().CreateArticleMessage(m["text"].(string), m["articleID"].(string), m["articleTitle"].(string), m["articleCoverPhoto"].(string), m["articleDetailsUrl"].(string), m["officialID"].(string), m["officialName"].(string), m["officialFaceUrl"].(string), operationID)
	wsRouter.GlobalSendMessage(EventData{cleanUpfuncName(runFuncName()), 0, "", msg, operationID})
}

func (wsRouter *WsFuncRouter) CreateWalletTransferMessage(input string, operationID string) {
	m := make(map[string]interface{})
	if err := json.Unmarshal([]byte(input), &m); err != nil {
		log.Info(operationID, utils.GetSelfFuncName(), "unmarshal failed", input, err.Error())
		wsRouter.GlobalSendMessage(EventData{cleanUpfuncName(runFuncName()), StatusBadParameter, "unmarshal failed", "", operationID})
	...
	userWorker := open_im_sdk.GetUserWorkerNew(wsRouter.uId)
	if !wsRouter.checkResourceLoadingAndKeysIn(userWorker, input, operationID, runFuncName(), m, "text", "transactionID", "senderID", "amount", "currency") {
		return
	}
	msg := userWorker.Conversation().CreateWalletTransferMessage(m["text"].(string), m["transactionID"].(string), m["senderID"].(string), m["currency"].(string), operationID, m["amount"].(string))
	wsRouter.GlobalSendMessage(EventData{cleanUpfuncName(runFuncName()), 0, "", msg, operationID})
}

func (wsRouter *WsFuncRouter) CreateWoomMessage(input string, operationID string) {
	m := make(map[string]interface{})
	if err := json.Unmarshal([]byte(input), &m); err != nil {
		log.Info(operationID, utils.GetSelfFuncName(), "unmarshal failed", input, err.Error())
		wsRouter.GlobalSendMessage(EventData{cleanUpfuncName(runFuncName()), StatusBadParameter, "unmarshal failed", "", operationID})
	...
	userWorker := open_im_sdk.GetUserWorkerNew(wsRouter.uId)
	if !wsRouter.checkResourceLoadingAndKeysIn(userWorker, input, operationID, runFuncName(), m, "fileId", "coverUrl", "mediaUrl", "userId", "nickname", "faceUrl", "desc") {
		return
	}
	msg := userWorker.Conversation().CreateWoomMessage(m["fileId"].(string), m["coverUrl"].(string), m["mediaUrl"].(string), m["userId"].(string), m["nickname"].(string), m["faceUrl"].(string), operationID, m["desc"].(string))
	wsRouter.GlobalSendMessage(EventData{cleanUpfuncName(runFuncName()), 0, "", msg, operationID})
}

func (wsRouter *WsFuncRouter) CreateOfficialAccountMessage(input string, operationID string) {
	m := make(map[string]interface{})
	if err := json.Unmarshal([]byte(input), &m); err != nil {
		log.Info(operationID, utils.GetSelfFuncName(), "unmarshal failed", input, err.Error())
		wsRouter.GlobalSendMessage(EventData{cleanUpfuncName(runFuncName()), StatusBadParameter, "unmarshal failed", "", operationID})
	...
	userWorker := open_im_sdk.GetUserWorkerNew(wsRouter.uId)
	if !wsRouter.checkResourceLoadingAndKeysIn(userWorker, input, operationID, runFuncName(), m, "text", "officialID", "nickname", "faceURL", "bio", "followTime", "type") {
		return
	}
	msg := userWorker.Conversation().CreateOfficialAccountMessage(m["text"].(string), int32(m["officialID"].(float64)), m["nickname"].(string), m["faceURL"].(string), m["bio"].(string), int32(m["followTime"].(float64)), int32(m["type"].(float64)), operationID)
	wsRouter.GlobalSendMessage(EventData{cleanUpfuncName(runFuncName()), 0, "", msg, operationID})
}
...
//func (wsRouter *WsFuncRouter) SendCallingMessage(input string, operationID string) {
//
//	m := make(map[string]interface{})
//	if err := json.Unmarshal([]byte(input), &m); err != nil {
//		log.Info(operationID, utils.GetSelfFuncName(), "unmarshal failed", input, err.Error())
...
//	common.JsonUnmarshalAndArgsValidate(m["message"].(string), &s, &sc, operationID)
//	sc.clientMsgID = s.ClientMsgID
//	userWorker.Conversation().SendCallingMessage(&sc, m["message"].(string), m["roomID"].(string), m["roomType"].(int64), m["offlinePushInfo"].(string), operationID)
//
//
//}

// SendBroadCastMessage
func (wsRouter *WsFuncRouter) SendBroadCastMessage(input string, operationID string) {

	m := make(map[string]interface{})
	if err := json.Unmarshal([]byte(input), &m); err != nil {
		log.Info(operationID, utils.GetSelfFuncName(), "unmarshal failed", input, err.Error())
		wsRouter.GlobalSendMessage(EventData{cleanUpfuncName(runFuncName()), StatusBadParameter, "unmarshal failed" + err.Error(), "", operationID})
		return
	}
	var reciverIDslice []string
	var groupIDslice []string
	json.Unmarshal([]byte(m["recvIDs"].(string)), &reciverIDslice)
	json.Unmarshal([]byte(m["groupIDs"].(string)), &groupIDslice)

	log.Error("Sandman Broadcast Receivers SendBroadcastMsg WS", reciverIDslice, groupIDslice)
	...
	sc.funcName = runFuncName()
	userWorker := open_im_sdk.GetUserWorkerNew(wsRouter.uId)
	if !wsRouter.checkResourceLoadingAndKeysIn(userWorker, input, operationID, runFuncName(), m, "message", "recvIDs", "offlinePushInfo") {
		return
	}
	s := sdk_struct.MsgStruct{}
	common.JsonUnmarshalAndArgsValidate(m["message"].(string), &s, &sc, operationID)
	sc.clientMsgID = s.ClientMsgID
	userWorker.Conversation().SendBroadCastMessageV3(&sc, m["message"].(string), reciverIDslice, groupIDslice, m["actionType"].(string), m["offlinePushInfo"].(string), operationID)

}
func (wsRouter *WsFuncRouter) GetBroadcastMessageList(input string, operationID string) {
	...
	userWorker := open_im_sdk.GetUserWorkerNew(wsRouter.uId)
	if !wsRouter.checkResourceLoadingAndKeysIn(userWorker, input, operationID, runFuncName(), m, "pageNumber") {
		return
	}
	...
	sc.operationID = operationID
	sc.funcName = runFuncName()
	userWorker.Conversation().GetBroadcastMessageList(&sc, int32(m["pageNumber"].(float64)), operationID)

}

func (wsRouter *WsFuncRouter) GetBroadcastMessageReceiverList(input string, operationID string) {
	...
	userWorker := open_im_sdk.GetUserWorkerNew(wsRouter.uId)
	if !wsRouter.checkResourceLoadingAndKeysIn(userWorker, input, operationID, runFuncName(), m, "clientMsgID") {
		return
	}
	...
	sc.operationID = operationID
	sc.funcName = runFuncName()
	userWorker.Conversation().GetBroadcastMessageReceiverList(&sc, m["clientMsgID"].(string), operationID)

}

func (wsRouter *WsFuncRouter) GetBroadcastList(input string, operationID string) {
	var sc SendCallback
	sc.uid = wsRouter.uId
	sc.operationID = operationID
	sc.funcName = runFuncName()
	...
	userWorker.Conversation().GetBroadcastList(&sc, sc.uid, operationID)
}
...
func (c *ConversationCallback) OnReloadAllConversation(conversationList string) {
	var ed EventData
	ed.Event = cleanUpfuncName(runFuncName())
	ed.ErrCode = 0
	ed.Data = conversationList
...
func (wsRouter *WsFuncRouter) GetAllConversationListByType(input string, operationID string) {
	userWorker := open_im_sdk.GetUserWorkerNew(wsRouter.uId)
	m := make(map[string]interface{})
	if err := json.Unmarshal([]byte(input), &m); err != nil {
		log.Info(operationID, utils.GetSelfFuncName(), "unmarshal failed", input, err.Error())
	...
	if !wsRouter.checkResourceLoadingAndKeysIn(userWorker, input, operationID, runFuncName(), m, "conversationType") {
		return
	}
	conversationTypeIntArray := make([]int32, 0)
	for _, f := range m["conversationType"].([]interface{}) {
		conversationTypeIntArray = append(conversationTypeIntArray, int32(f.(float64)))
	}

	userWorker.Conversation().GetAllConversationListByType(&BaseSuccessFailed{runFuncName(), operationID, wsRouter.uId}, conversationTypeIntArray, operationID)
}
...
func (wsRouter *WsFuncRouter) GetRecentForwardConversations(m string, operationID string) {
	userWorker := open_im_sdk.GetUserWorkerNew(wsRouter.uId)
	log.Error("GetRecentForwardConversations", "GetRecentForwardConversations called")
	if !wsRouter.checkResourceLoadingAndKeysIn(userWorker, "", operationID, runFuncName(), nil) {
		return
	}
	log.Error("GetRecentForwardConversations", "GetRecentForwardConversations ")
	userWorker.Conversation().GetRecentForwardConversations(&BaseSuccessFailed{runFuncName(), operationID, wsRouter.uId}, operationID)
}
...
func (wsRouter *WsFuncRouter) GetHistoryMessageListByStartMsgId(getMessageOptions string, operationID string) {
	var sc SendCallback
	sc.uid = wsRouter.uId
	sc.funcName = runFuncName()
	userWorker := open_im_sdk.GetUserWorkerNew(wsRouter.uId)
	...
	userWorker.Conversation().GetHistoryMessageListByStartMsgId(&BaseSuccessFailed{runFuncName(), operationID, wsRouter.uId}, getMessageOptions, operationID)
}
...
func (wsRouter *WsFuncRouter) MarkOfficialMessageAsRead(input string, operationID string) {
	m := make(map[string]interface{})
	if err := json.Unmarshal([]byte(input), &m); err != nil {
		log.Info(operationID, utils.GetSelfFuncName(), "unmarshal failed", input, err.Error())
		wsRouter.GlobalSendMessage(EventData{cleanUpfuncName(runFuncName()), StatusBadParameter, "unmarshal failed", "", operationID})
	...
	userWorker.Conversation().MarkOfficialMessageAsRead(&BaseSuccessFailed{runFuncName(), operationID, wsRouter.uId}, m["userID"].(string), m["msgIDList"].(string), operationID)
}
...
// func (wsRouter *WsFuncRouter) GetUserIPandStatus(params string, operationID string) {
// 	userWorker := open_im_sdk.GetUserWorkerNew(wsRouter.uId)
// 	if !wsRouter.checkResourceLoadingAndKeysIn(userWorker, params, operationID, runFuncName(), nil) {
// 		return
// 	}
// 	userWorker.Conversation().GetUserIPandStatus(&BaseSuccessFailed{runFuncName(), operationID, wsRouter.uId}, params, operationID)
// }

func (wsRouter *WsFuncRouter) TestSendMessage(input string, operationID string) {
	m := make(map[string]interface{})
	if err := json.Unmarshal([]byte(input), &m); err != nil {
		log.Info(operationID, utils.GetSelfFuncName(), "unmarshal failed", input, err.Error())
		wsRouter.GlobalSendMessage(EventData{cleanUpfuncName(runFuncName()), StatusBadParameter, "unmarshal failed", "", operationID})
	...
	userWorker.Conversation().TestSendMessage(&sc, m, operationID)
