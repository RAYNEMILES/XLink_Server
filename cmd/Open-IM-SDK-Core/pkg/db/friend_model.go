...
func (d *DataBase) DeleteFriendForUser(userID, friendUserID string) error {
	//d.mRWMutex.Lock()
	//defer d.mRWMutex.Unlock()
	return utils.Wrap(d.conn.Where("owner_user_id=? and friend_user_id=?", userID, friendUserID).Delete(&model_struct.LocalFriend{}).Error, "DeleteFriend failed")
}
...
func (d *DataBase) GetAllFriendList() ([]*model_struct.LocalFriend, error) {
	...
	err := utils.Wrap(d.conn.Where("owner_user_id = ? and create_time > 0", d.loginUserID).Find(&friendList).Error,
