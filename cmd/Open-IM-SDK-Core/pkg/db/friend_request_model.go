...
func (d *DataBase) GetFriendApplication() ([]*model_struct.LocalFriendRequest, error) {
	//d.mRWMutex.Lock()
	//defer d.mRWMutex.Unlock()
	var friendRequestList []model_struct.LocalFriendRequest
	err := utils.Wrap(d.conn.Order("create_time DESC").Find(&friendRequestList).Error, "GetFriendApplication failed")
	...
	return transfer, utils.Wrap(err, "GetFriendApplication failed")
}
