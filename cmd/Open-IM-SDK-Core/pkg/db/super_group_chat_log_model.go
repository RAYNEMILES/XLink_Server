...
func (d *DataBase) SuperGroupBatchInsertMessageList(MessageList []*model_struct.LocalChatLog, groupID string) error {
	...
	go func() {
		...
		for _, chatLog := range MessageList {
			localSeqSynced.ConversationID = utils.GetConversationIDBySessionType(chatLog.RecvID, constant.GroupChatType)
...
func (d *DataBase) SuperGroupInsertMessage(Message *model_struct.LocalChatLog, groupID string) error {
	...
	go func() {
		localSeqSynced := &model_struct.LocalSeqSynced{}
		localSeqSynced.ConversationID = utils.GetConversationIDBySessionType(Message.RecvID, constant.GroupChatType)
...
func (d *DataBase) SuperGroupSearchMessageByKeyword(contentType []int, hasLink int, keywordList []string, keywordListMatchType int, sourceID string, startTime, endTime int64, sessionType, offset, count int) (result []*model_struct.LocalChatLog, err error) {
	//d.mRWMutex.Lock()
	//defer d.mRWMutex.Unlock()
...
func (d *DataBase) SuperGroupSearchMessageByContentType(contentType []int, hasLink int, sourceID string, startTime, endTime int64, sessionType, offset, count int) (result []*model_struct.LocalChatLog, err error) {
	var messageList []model_struct.LocalChatLog
	var condition string
	if hasLink == 0 {
		...
		condition = fmt.Sprintf("session_type=%d And recv_id==%q And send_time between %d and %d AND status <=%d AND has_link=%d And content_type IN ?", sessionType, sourceID, startTime, endTime, constant.MsgStatusSendFailed, hasLink-1)
	}
...
func (d *DataBase) SuperGroupSearchMessageByContentTypeAndKeyword(contentType []int, hasLink int, keywordList []string, keywordListMatchType int, startTime, endTime int64, groupID string) (result []*model_struct.LocalChatLog, err error) {
	var messageList []model_struct.LocalChatLog
	var condition string
	var subCondition string
	if keywordListMatchType == constant.KeywordMatchOr {
...
func (d *DataBase) SuperGroupGetMessage(msg *sdk_struct.MsgStruct) (*model_struct.LocalChatLog, error) {
	...
	return &c, utils.Wrap(d.conn.Table(utils.GetSuperGroupTableName(msg.GroupID)).Where("client_msg_id = ?",
		msg.ClientMsgID).Take(&c).Error, "GetMessage failed, "+msg.ClientMsgID)
}
...
func (d *DataBase) SuperGroupGetMessageListByStartMsgId(sourceID string, sessionType int, startTime, endTime int64) (result []*model_struct.LocalChatLog, err error) {
	//d.mRWMutex.Lock()
	//defer d.mRWMutex.Unlock()
	var condition, timeOrder, seqOrder string
	timeOrder = "send_time DESC"
	seqOrder = "seq DESC"
	condition = " recv_id = ? AND status <=? And session_type = ? And send_time < ? AND send_time > ?"
	log.NewInfo("", utils.GetSelfFuncName(), condition, timeOrder, seqOrder)

	err = utils.Wrap(d.conn.Table(utils.GetSuperGroupTableName(sourceID)).Where(condition, sourceID, constant.MsgStatusSendFailed, sessionType, startTime, endTime).
		Order(timeOrder).Find(&result).Error, "GetMessageList failed")
	return result, err
}
func (d *DataBase) SuperGroupGetMessageListByStartMsgIdNoTime(sourceID string, sessionType int, endTime int64) (result []*model_struct.LocalChatLog, err error) {
	d.initSuperLocalChatLog(sourceID)
	var condition, timeOrder, seqOrder string
	timeOrder = "send_time DESC"
	seqOrder = "seq DESC"
	condition = "recv_id = ? AND status <=? AND session_type = ? AND send_time > ?"
	log.NewInfo("", utils.GetSelfFuncName(), condition, timeOrder, seqOrder)

	err = utils.Wrap(d.conn.Table(utils.GetSuperGroupTableName(sourceID)).Where(condition, sourceID, constant.MsgStatusSendFailed, sessionType, endTime).
		Order(timeOrder).Find(&result).Error, "GetMessageList failed")
	return result, err
}
...
func (d *DataBase) SuperGroupGetMsgSeqByClientMsgID(clientMsgID string, groupID string) (uint32, error) {
	...
	//defer d.mRWMutex.Unlock()
	var seq uint32
	//err := utils.Wrap(d.conn.Debug().Table(utils.GetSuperGroupTableName(groupID)).Select("seq").Where("client_msg_id=?", clientMsgID).First(&seq).Error, utils.GetSelfFuncName()+" failed")
