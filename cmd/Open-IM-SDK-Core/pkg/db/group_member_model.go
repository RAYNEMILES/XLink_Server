...
func (d *DataBase) GetGroupMemberListSplit(groupID string, filter int32, offset, count int, nameSearch string) ([]*model_struct.LocalGroupMember, error) {
	//d.mRWMutex.Lock()
	//defer d.mRWMutex.Unlock()
	var groupMemberList []model_struct.LocalGroupMember
	var err error
	if filter == 0 {
		err = d.conn.Where("group_id = ? And role_level > ? And nickname like ?", groupID, filter, "%"+nameSearch+"%").Order("case when role_level=2 then 0 when role_level=3 then 1 else 2 end, role_level DESC").Order("join_time ASC").Offset(offset).Limit(count).Find(&groupMemberList).Error
	} else if filter == constant.GroupOrdinaryUsers || filter == constant.GroupOwner || filter == constant.GroupAdmin {
		err = d.conn.Where("group_id = ? And role_level = ? And nickname like ?", groupID, filter, "%"+nameSearch+"%").Order("join_time ASC").Offset(offset).Limit(count).Find(&groupMemberList).Error
	} else if filter == constant.GroupAdminAndOrdinaryUsers {
		err = d.conn.Where("group_id = ? And ( role_level = 1 OR role_level = 3 ) And nickname like ?", groupID, "%"+nameSearch+"%").Order("case when role_level=2 then 0 when role_level=3 then 1 else 2 end, role_level DESC").Order("join_time ASC").Offset(offset).Limit(count).Find(&groupMemberList).Error
	} else {
...
func (d *DataBase) GetGroupOwnerByGroupID(groupID string) (*model_struct.LocalGroupMember, error) {
	//d.mRWMutex.Lock()
	//defer d.mRWMutex.Unlock()
	var groupMember model_struct.LocalGroupMember
	err := d.conn.Where("group_id = ?  AND role_level = ?", groupID, constant.GroupOwner).Limit(1).Take(&groupMember).Error
	if err != nil {
		return nil, err
	}
...
func (d *DataBase) UpsertGroupMember(groupMember *model_struct.LocalGroupMember) error {
	//d.mRWMutex.Lock()
	//defer d.mRWMutex.Unlock()
	err := d.conn.Clauses(clause.OnConflict{
		Columns:   []clause.Column{{Name: "group_id"}, {Name: "user_id"}},
		DoUpdates: clause.AssignmentColumns([]string{"nickname", "user_group_face_url", "role_level", "join_time", "join_source", "inviter_user_id", "mute_end_time", "operator_user_id", "ex", "attached_info"}),
...
func (d *DataBase) DeleteGroupAllMembers(groupID string) error {
	//d.mRWMutex.Lock()
	//defer d.mRWMutex.Unlock()
	d.UpdateGroupUpdateVersionByID(groupID, 0)
	groupMember := model_struct.LocalGroupMember{}
