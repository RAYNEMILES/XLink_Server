...
func MergeUserResult(publicList []*sdk.PublicUserInfo, friendList []*model_struct.LocalFriend, blackList []*model_struct.LocalBlack) []api.FullUserInfo {
	m := make(map[string]api.FullUserInfo, 0)
	...
	for _, v := range friendList {
		if t, ok := m[v.FriendUserID]; ok {
			...
			node := api.FullUserInfo{PublicInfo: &sdk.PublicUserInfo{}}
			node.FriendInfo = v
