...
func InOutDoTest(uid, tk, ws, api string) {
	...
	var testConversationListener open_im_sdk_callback.OnConversationListener
	open_im_sdk.SetConversationListener(testConversationListener)

	//var testUser userCallback
	var testUserListener open_im_sdk_callback.OnUserListener
	open_im_sdk.SetUserListener(testUserListener)
	...
	var groupListener open_im_sdk_callback.OnGroupListener
...
func ReliabilityInitAndLogin(index int, uid, tk, ws, api string) {
	...
	//var testConversation conversationCallBack
	var testConversation open_im_sdk_callback.OnConversationListener
	lg.SetConversationListener(testConversation)

	//var testUser userCallback
	var testUser open_im_sdk_callback.OnUserListener
...
func PressInitAndLogin(index int, uid, tk, ws, api, dataDir string) {
	var cf sdk_struct.IMConfig
	cf.ApiAddr = api
	cf.WsAddr = ws
	cf.Platform = 1
	cf.DataDir = dataDir
	cf.LogLevel = LogLevel
	X.Println("", "DoReliabilityTest", uid, tk, ws, api, dataDir)
	log.Info("", "DoReliabilityTest", uid, tk, ws, api, dataDir)

	operationID := utils.OperationIDGenerator()
	var testinit testInitLister
	lg := new(login.LoginMgr)
	X.Println(operationID, "new login ", lg)
	log.Info(operationID, "new login ", lg)
	...
	X.Println(operationID, "InitSDK ", sdk_struct.SvrConf)
	log.Info(operationID, "InitSDK ", sdk_struct.SvrConf)
	...
	for {
		if callback.errCode == 1 {
			X.Println(operationID, "login ok ", uid, uid, callback.errCode, callback.errMsg, callback.successData)
			log.Warn(operationID, "login ok ", uid, uid, callback.errCode, callback.errMsg, callback.successData)
			return
		} else {
			//X.Println(operationID, "waiting login...", uid, callback.errCode, callback.errMsg)
			log.Warn(operationID, "waiting login...", uid, callback.errCode, callback.errMsg)
		}
		//		log.Warn(operationID, "waiting login...", uid)
		//	time.Sleep(100 * time.Millisecond)
	}
...
func (t *TestSendMsgCallBack) OnError(errCode int32, errMsg string) {
	X.Println(t.OperationID, "test_openim: send msg failed: ", errCode, errMsg, "t.msgID", t.msgID, "t.msg", t.msg)
	log.Error(t.OperationID, "test_openim: send msg failed: ", errCode, errMsg, t.msgID, t.msg)
	SendMsgMapLock.Lock()
	defer SendMsgMapLock.Unlock()
	//PT.SendFailedAllMsg[t.msgID] = t.sendID + t.recvID
	t.result = false
	PT.SendFailedMsgNum++
	X.Println("total failed message number:", PT.SendFailedMsgNum, "total success message number:", PT.SendSuccMsgNum)
}

func (t *TestSendMsgCallBack) OnSuccess(data string) {
	X.Println(t.OperationID, "test_openim: send msg success: |", "t.msgID", t.msgID, "t.msg", t.msg, "t.data", data)
	log.Info(t.OperationID, "test_openim: send msg success: |", "t.msgID", t.msgID, "t.msg", t.msg, "t.data", data)
	SendMsgMapLock.Lock()
	defer SendMsgMapLock.Unlock()
	t.result = true
	PT.SendSuccMsgNum++
	//PT.SendSuccAllMsg[t.msgID] = &SendRecvTime{
	//	SendTime:             t.sendTime,
	//	SendSeccCallbackTime: utils.GetCurrentTimestampByMill(),
	//	RecvTime:             t.recvTime,
	//	SendIDRecvID:         t.sendID + "|" + t.recvID + "|" + t.groupID,
	//}
	X.Println("total failed message number:", PT.SendFailedMsgNum, "total success message number:", PT.SendSuccMsgNum)
