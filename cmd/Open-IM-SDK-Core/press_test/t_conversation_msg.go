...
func (m *MsgListenerCallBak) OnRecvNewMessage(msg string) {
	...
	if err != nil {
		...
		//PT.RecvAllMsg[mm.ClientMsgID] = &t
		PT.RecvMsgNum++
		log.Info("", "OnRecvNewMessage  callback", mm.ClientMsgID, mm.SendID, mm.RecvID, PT.RecvMsgNum)
	}
}
...
func (c conversationCallBack) OnReloadAllConversation(conversationList string) {
	log.Info("", "OnReloadAllConversation returnList is", conversationList)
}
...
func DoTestSendMsg(index int, sendId, recvID string, groupID string, idx string) {
	...
	//PT.SendSuccAllMsg[testSendMsg.msgID] = &x
	PT.SendSuccMsgNum++
}
