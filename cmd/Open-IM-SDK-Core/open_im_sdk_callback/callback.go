...
type OnGroupListener interface {
	...
	OnGroupMembersSyncedStarted(groupInfo string)
	OnGroupMembersSyncedFinished(groupInfo string)
}
...
type OnConversationListener interface {
	...
	OnReloadAllConversation(conversationList string)
}
...
type OnUserListener interface {
	...
	OnMomentNotification(momentInfoIDAction string)
}
