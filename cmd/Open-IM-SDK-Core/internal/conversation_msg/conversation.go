...
import (
	_ "Open_IM/cmd/Open-IM-SDK-Core/internal/common"
...
func (c *Conversation) getAllConversationListByType(callback open_im_sdk_callback.Base, conversationType []int32, operationID string) sdk.GetAllConversationListCallback {
	conversationList, err := c.db.GetAllConversationListByType(conversationType)
	common.CheckDBErrCallback(callback, err, operationID)
	return conversationList
}
...
func (c *Conversation) setConversationRecvMessageOpt(callback open_im_sdk_callback.Base, conversationIDList []string, opt int, operationID string) {
	...
	for _, conversationID := range conversationIDList {
		...
		go func() {
			if localConversation.RecvMsgOpt == 1 && localConversation.GroupID != "" {
				paramsGetMaxSeq := api.ParamsGetMaxSeq{}
				paramsGetMaxSeq.OperationID = operationID
				paramsGetMaxSeq.OpUserID = c.loginUserID
				paramsGetMaxSeq.GroupID = localConversation.GroupID

				response := map[string]interface{}{}
				response["maxSeq"] = 0
				response[paramsGetMaxSeq.GroupID] = 0
				c.p.PostReturn(constant.GetMaxSeqRouter, paramsGetMaxSeq, &response)

				if seqNumberMaxInterf, ok := response[paramsGetMaxSeq.GroupID]; ok {
					seqNumberMax := seqNumberMaxInterf.(float64)
					localSeqSynced := &model_struct.LocalSeqSynced{}
					localSeqSynced.ConversationID = utils.GetConversationIDBySessionType(paramsGetMaxSeq.GroupID, constant.GroupChatType)
					localSeqSynced.Seq = uint32(seqNumberMax)
					if localSeqSynced.Seq > 0 {
						c.db.UpdateLocalSeqNumber(localSeqSynced)
					}
				}
	...
	go c.SyncConversations(operationID)
}
...
func (c *Conversation) setOneConversationPinned(callback open_im_sdk_callback.Base, conversationID string, isPinned bool, operationID string) {
	...
	apiReq.PinnedTime = time.Now().Unix()
	c.setConversation(callback, apiReq, conversationID, localConversation, operationID)
	go c.SyncConversations(operationID)
}
...
func (c *Conversation) getOneConversation(callback open_im_sdk_callback.Base, sourceID string, sessionType int32, operationID string) *model_struct.LocalConversation {
	...
	if err == nil {
		...
		case constant.SingleChatType:
			...
			faceUrl, name, accountStatus, err, isFromSvr := c.friend.GetUserNameAndFaceUrlByUid(sourceID, operationID)
			...
			newConversation.AccountStatus = accountStatus
		case constant.BroadcastChatType:
			newConversation.UserID = c.loginUserID
			faceUrl, name, accountStatus, err, isFromSvr := c.friend.GetUserNameAndFaceUrlByUid(c.loginUserID, operationID)
			//	faceUrl, name, err := c.cache.GetUserNameAndFaceURL(sourceID, operationID)
			common.CheckDBErrCallback(callback, err, operationID)
			if isFromSvr {
				c.cache.Update(sourceID, faceUrl, name)
			}
			newConversation.ShowName = "Broadcast"
			newConversation.FaceURL = faceUrl
			newConversation.AccountStatus = accountStatus
...
func (c *Conversation) deleteConversation(callback open_im_sdk_callback.Base, conversationID, operationID string) {
	...
	if lc.ConversationType == constant.OfficialArticlesChatType {
		err = c.db.DeleteConversation(conversationID)
		common.CheckDBErrCallback(callback, err, operationID)
		c.db.MoveConversationToArchived(lc)
...
func (c *Conversation) getOneServerConversation(operationID, conversationID string) (server_api_params.GetConversationResp, error) {
	log.NewInfo(operationID, utils.GetSelfFuncName())
	var req server_api_params.GetConversationReq
	var resp server_api_params.GetConversationResp
	req.OwnerUserID = c.loginUserID
	req.ConversationID = conversationID
	req.OperationID = operationID
	err := c.p.PostReturn(constant.GetConversationRouter, req, &resp.Conversation)
	if err != nil {
...
func (c *Conversation) SyncConversations(operationID string) {
	...
	for _, index := range aInBNot {
		...
		case constant.SingleChatType, constant.NotificationChatType:
			...
			faceUrl, name, accountStatus, err, isFromSvr := c.friend.GetUserNameAndFaceUrlByUid(conversation.UserID, operationID)
			if err != nil {
		...
		newConversation.PinnedTime = conversation.PinnedTime
...
func (c *Conversation) SyncOneConversation(conversationID, operationID string) {
	...
	// todo
	resp, err := c.getOneServerConversation(operationID, conversationID)
	if err != nil {
		log.NewError(operationID, utils.GetSelfFuncName(), "getOneServerConversation failed: ", err.Error(), conversationID, c.loginUserID)
		return
	}
	localConversation := common.TransferToOneLocalConversation(resp)
	log.NewInfo(operationID, utils.GetSelfFuncName(), "TransferToOneLocalConversation:", localConversation.ConversationID, localConversation.IsNotInGroup)
	err = c.db.UpdateConversation(localConversation)
	if err != nil {
		log.NewError(operationID, utils.GetSelfFuncName(), "UpdateConversation failed: ", err.Error())
	}
	err = c.db.UpdateColumnsConversation(conversationID, map[string]interface{}{
		"is_pinned":       localConversation.IsPinned,
		"is_private_chat": localConversation.IsPrivateChat,
		"is_not_in_group": localConversation.IsNotInGroup,
	})

	if err != nil {
		log.NewError(operationID, utils.GetSelfFuncName(), "UpdateColumnsConversation failed: ", err.Error())
		return
	}
	log.NewInfo(operationID, utils.GetSelfFuncName(), "SyncOneConversation success: ", conversationID)
}
...
func (c *Conversation) getHistoryMessageListByStartMsgId(callback open_im_sdk_callback.Base, req sdk.GetHistoryMessageListByStartMsgIdParams, operationID string) sdk.GetHistoryMessageListCallback {
	t := time.Now()
	t2 := time.Now()
	var sourceID string
	var conversationID string
	...
	var msg sdk_struct.MsgStruct
	var notStartTime bool
	if req.EndClientMsgID == "" {
		callback.OnError(constant.ErrArgs.ErrCode, "must have end client msg id")
		return nil
	}
	if req.ConversationID != "" {
		conversationID = req.ConversationID
		...
		sessionType = int(lc.ConversationType)
		msg.SessionType = lc.ConversationType
		msg.ClientMsgID = req.EndClientMsgID
		m, err := c.db.GetMessageController(&msg)
		common.CheckDBErrCallback(callback, err, operationID)
		endTime = m.SendTime
		if req.StartClientMsgID == "" {
	...
	log.Debug(operationID, "Assembly parameters cost time", time.Since(t))
	t = time.Now()
	log.Info(operationID, "sourceID:", sourceID, "startTime:", startTime, "not end_time", notStartTime)
	var err error
	if notStartTime {
		list, err = c.db.GetMessageListByStartMsgIdNoStartTimeController(sourceID, sessionType, endTime)
	} else {
		list, err = c.db.GetMessageListByStartMsgIdController(sourceID, sessionType, startTime, endTime)
	}
	log.Debug(operationID, "db cost time", time.Since(t))
	common.CheckDBErrCallback(callback, err, operationID)
	t = time.Now()
	...
	sort.Sort(messageList)
	log.Debug(operationID, "sort cost time", time.Since(t))
	log.NewError(operationID, "---- api cost time", time.Since(t2))
	return sdk.GetHistoryMessageListCallback(messageList)
...
func (c *Conversation) getAdvancedHistoryMessageList(callback open_im_sdk_callback.Base, req sdk.GetAdvancedHistoryMessageListParams, operationID string, isReverse bool) sdk.GetAdvancedHistoryMessageListCallback {
	...
	if len(list) < req.Count && sessionType == constant.SuperGroupChatType {
		var minSeq uint32
		resp, err := c.SendReqWaitResp(&sdk2.GetMaxAndMinSeqReq{UserID: c.loginUserID, GroupIDList: []string{sourceID}}, constant.WSGetNewestSeq, 30, 3, c.loginUserID, operationID)
		if err != nil {
...
func (c *Conversation) pullMessageAndReGetHistoryMessages(sourceID string, seqList []uint32, notStartTime, isReverse bool, count, sessionType int, startTime int64, list *[]*model_struct.LocalChatLog, messageListCallback *sdk.GetAdvancedHistoryMessageListCallback, operationID string) {
	var pullMsgReq sdk2.PullMessageBySeqListReq
	pullMsgReq.UserID = c.loginUserID
	pullMsgReq.GroupSeqList = make(map[string]*sdk2.SeqList, 0)
	pullMsgReq.GroupSeqList[sourceID] = &sdk2.SeqList{SeqList: seqList}
	...
	if err != nil {
		...
		var pullMsgResp sdk2.PullMessageBySeqListResp
...
func (c *Conversation) pullMessageIntoTable(pullMsgData []*sdk2.MsgData, operationID string) {
	...
	for _, v := range pullMsgData {
		...
		if v.SendID == c.loginUserID { //seq
			...
			if oldMessage, err := c.db.GetMessageController(msg); err != nil { //Deduplication operation
				log.NewWarn("test", "trigger msg is ", msg.SenderNickname, msg.SenderFaceURL)
				insertMsg = append(insertMsg, c.msgStructToLocalChatLog(msg))
				switch msg.ContentType {
				case constant.Revoke:
					msgRevokeList = append(msgRevokeList, msg)
				...
				if oldMessage.Seq == 0 {
					...
					exceptionMsg = append(exceptionMsg, c.msgStructToLocalErrChatLog(msg))
					log.NewWarn(operationID, "Deduplication operation ", *c.msgStructToLocalErrChatLog(msg))
				}
			}
		}
	}
	...
	if err1 != nil {
		...
		for _, v := range insertMsg {
			...
			if e != nil {
				...
				log.NewWarn(operationID, "InsertMessage operation ", "chat err log: ", errChatLog, "chat log: ", v, e.Error())
			}
		}
	}
	...
	//	log.NewWarn(operationID, "exceptionMsg show: ", *v)
	//}
...
func (c *Conversation) revokeOneMessage(callback open_im_sdk_callback.Base, req sdk.RevokeMessageParams, operationID string) {
	...
	resp, _ := c.InternalSendMessage(callback, (*sdk_struct.MsgStruct)(&req), recvID, groupID, operationID, &sdk2.OfflinePushInfo{}, false, options)
	req.ServerMsgID = resp.ServerMsgID
	req.SendTime = resp.SendTime
	req.SortTime = message.SendTime
	...
	if localMessage.ContentType == constant.File || localMessage.ContentType == constant.Video || localMessage.ContentType == constant.Picture {
		c.StopUpload(localMessage.ClientMsgID)
	}
...
func (c *Conversation) newRevokeOneMessage(callback open_im_sdk_callback.Base, req sdk.RevokeMessageParams, operationID string) {
	...
	utils.SetSwitchFromOptions(options, constant.IsUnreadCount, false)
	utils.SetSwitchFromOptions(options, constant.IsOfflinePush, false)
	resp, _ := c.InternalSendMessage(callback, &s, recvID, groupID, operationID, &sdk2.OfflinePushInfo{}, false, options)
...
func (c *Conversation) typingStatusUpdate(callback open_im_sdk_callback.Base, recvID, msgTip, operationID string) {
	...
	utils.SetSwitchFromOptions(options, constant.IsUnreadCount, false)
	utils.SetSwitchFromOptions(options, constant.IsOfflinePush, false)
	c.InternalSendMessage(callback, &s, recvID, "", operationID, &sdk2.OfflinePushInfo{}, true, options)
...
func (c *Conversation) markC2CMessageAsRead(callback open_im_sdk_callback.Base, msgIDList sdk.MarkC2CMessageAsReadParams, userID, operationID string) {
	...
	resp, _ := c.InternalSendMessage(callback, &s, userID, "", operationID, &sdk2.OfflinePushInfo{}, false, options)
	s.ServerMsgID = resp.ServerMsgID
	s.SendTime = resp.SendTime
	...
	_ = common.TriggerCmdUpdateConversation(common.UpdateConNode{ConID: conversationID, Action: constant.UpdateLatestMessageChange, Args: []string{conversationID}}, c.GetCh())
	_ = common.TriggerCmdUpdateConversation(common.UpdateConNode{ConID: conversationID, Action: constant.UnreadCountSetZero, Args: []string{conversationID}}, c.GetCh())
...
func (c *Conversation) markGroupMessageAsRead(callback open_im_sdk_callback.Base, msgIDList sdk.MarkGroupMessageAsReadParams, groupID, operationID string) {
	...
	utils.SetSwitchFromOptions(options, constant.IsOfflinePush, false)
	//If there is an error, the coroutine ends, so judgment is not  required
	resp, _ := c.InternalSendMessage(callback, &s, "", groupID, operationID, &sdk2.OfflinePushInfo{}, false, options)
	s.ServerMsgID = resp.ServerMsgID
	s.SendTime = resp.SendTime
	s.Status = constant.MsgStatusFiltered
	msgStructToLocalChatLog(&localMessage, &s)
	err = c.db.SuperGroupInsertMessage(&localMessage, groupID)
	if err != nil {
		log.Error(operationID, "inset into chat log err", localMessage, s, err.Error())
	}

	err2 := c.db.UpdateGroupMessageHasReadController(newMessageIDList, groupID, constant.GroupChatType)
	if err2 != nil {
		log.Error(operationID, "update message has read error", newMessageIDList, groupID, err2.Error())
	}
	newMessages, err3 := c.db.GetMultipleMessageController(newMessageIDList, groupID, constant.GroupChatType)
	if err3 != nil {
		log.Error(operationID, "get messages error", newMessageIDList, groupID, err3.Error())
	}
	for _, v := range newMessages {
		attachInfo := sdk_struct.AttachedInfoElem{}
		_ = utils.JsonStringToStruct(v.AttachedInfo, &attachInfo)
...
func (c *Conversation) markGroupMessageAsReadOld(callback open_im_sdk_callback.Base, msgIDList sdk.MarkGroupMessageAsReadParams, groupID, operationID string) {
	conversationID, conversationType, err := c.getConversationTypeByGroupID(groupID)
	common.CheckAnyErrCallback(callback, 202, err, operationID)
	if len(msgIDList) == 0 {
		c.setOneConversationUnread(callback, conversationID, 0, operationID)
...
func (c *Conversation) deleteMessageFromSvr(callback open_im_sdk_callback.Base, s *sdk_struct.MsgStruct, operationID string) {
	...
	case constant.SingleChatType, constant.GroupChatType:
		...
		apiReq.ChatType = int64(s.SessionType)
		apiReq.OperationID = operationID
...
func (c *Conversation) deleteMessageFromLocalStorage(callback open_im_sdk_callback.Base, s *sdk_struct.MsgStruct, operationID string) {
	...
	if s.ContentType == constant.File || s.ContentType == constant.Video || s.ContentType == constant.Picture {
		c.StopUpload(s.ClientMsgID)
	}
...
func (c *Conversation) searchLocalMessages(callback open_im_sdk_callback.Base, searchParam sdk.SearchLocalMessagesParams, operationID string) (r sdk.SearchLocalMessagesCallback) {
	...
	if searchParam.ConversationID != "" {
		...
		if len(searchParam.MessageTypeList) != 0 && len(searchParam.KeywordList) == 0 {
			list, err = c.db.SearchMessageByContentTypeController(searchParam.MessageTypeList, searchParam.HasLink, sourceID, startTime, endTime, int(localConversation.ConversationType), offset, searchParam.Count)
		} else {
			...
			list, err = c.db.SearchMessageByKeywordController(newContentTypeList, searchParam.HasLink, searchParam.KeywordList, searchParam.KeywordListMatchType, sourceID, startTime, endTime, int(localConversation.ConversationType), offset, searchParam.Count)
		}
	} else {
		...
		list, err = c.db.SearchMessageByContentTypeAndKeywordController(searchParam.MessageTypeList, searchParam.HasLink, searchParam.KeywordList, searchParam.KeywordListMatchType, startTime, endTime, operationID)
	}
	common.CheckDBErrCallback(callback, err, operationID)
...
func (c *Conversation) setConversationNotification(msg *sdk2.MsgData, operationID string) {
	log.NewInfo(operationID, utils.GetSelfFuncName(), "args: ", msg.ClientMsgID, msg.ServerMsgID)
	go c.SyncConversations(operationID)
}

func (c *Conversation) DoNotification(msg *sdk2.MsgData) {
...
func (c *Conversation) delMsgBySeqSplit(seqList []uint32) error {
	...
	var delResp sdk2.DelMsgListResp
...
func (c *Conversation) deleteConversationAndMsgFromSvr(callback open_im_sdk_callback.Base, conversationID, operationID string) {
	...
	if local != nil {
		...
		apiReq.ChatType = int64(local.ConversationType)
		c.p.PostFatalCallback(callback, constant.DeleteMsgRouter, apiReq, nil, apiReq.OperationID)
		common.CheckArgsErrCallback(callback, err, operationID)
	}
}
...
func (c *Conversation) deleteAllMsgFromSvr(callback open_im_sdk_callback.Base, operationID string) {
	...
	apiReq.OperationID = operationID
	apiReq.SeqList = seqList
	apiReq.ChatType = constant.SingleChatType
	c.p.PostFatalCallback(callback, constant.DeleteMsgRouter, apiReq, nil, apiReq.OperationID)
}

func (c *Conversation) GetOfficialAccountConversationFormServer(operationID string) {
	var list []model_struct.LocalConversation
	log.NewInfo(operationID, utils.GetSelfFuncName())
	var apiReq server_api_params.OfficialAccountConversationRequest
	apiResponse := server_api_params.OfficialAccountConversationResponse{}
	apiReq.OperationID = operationID
	err := c.p.PostReturn(constant.GetOfficialAccountConversation, apiReq, &apiResponse.Articles)
	if err == nil {
		log.Error(operationID, "GetOfficialAccountConversationFormServer Response Len", len(apiResponse.Articles))
		serverCurrentFollowedOfficial := make(map[string]interface{})

		for _, article := range apiResponse.Articles {
			conversation, IsNewCreated := c.UpdateOfficialArticleAsLocalConv(article, operationID)
			serverCurrentFollowedOfficial[strconv.FormatInt(article.OfficialID, 10)] = article
			if IsNewCreated {
				list = append(list, conversation)
			}
		}
		localOfficialConv, _ := c.db.GetConversationsByType(6)

		for _, conversation := range localOfficialConv {
			_, ok := serverCurrentFollowedOfficial[conversation.UserID]
			if !ok {
				err := c.db.DeleteConversation(conversation.ConversationID)
				if err != nil {
					log.Error(operationID, "GetOfficialAccountConversationFormServer Delete Local Conver issue")
				}
				list = append(list, conversation)
		...
		if c.ConversationListener != nil {
			jsonStr := utils.StructToJsonString(list)
			log2.Println("utils.StructToJsonStrin", jsonStr, c.ConversationListener)
			c.ConversationListener.OnReloadAllConversation(jsonStr)
		}
...
// UpdateOfficialArticleAsLocalConv update official article in to local db as a conversation
func (c *Conversation) UpdateOfficialArticleAsLocalConv(article server_api_params.ArticleForLocalConv, operationID string) (model_struct.LocalConversation, bool) {
	var IsNewCreated = false
	conversationID := "official_art_" + strconv.FormatInt(article.OfficialID, 10)
	msgCreateTime := article.CreateTime
	if msgCreateTime < 10000000000 {
		msgCreateTime = msgCreateTime * 1000 //for fix unix time
	}
	lastMessage := map[string]interface{}{
		"clientMsgID":    "dbbe966585cfec2d9af17edd80166c2323964",
		"createTime":     msgCreateTime,
		"sendTime":       msgCreateTime,
		"sessionType":    5,
		"sendID":         strconv.FormatInt(article.OfficialID, 10),
		"msgFrom":        100,
		"contentType":    101,
		"platformID":     5,
		"senderNickname": article.OfficialName,
		"content":        article.Title,
	}

	jsonData, err := json.Marshal(lastMessage)
	if err != nil {
		log.Debug("could not marshal json: ", err.Error())
	}

	conversation, err := c.db.GetConversation(conversationID)
	if err != nil {
		archivedCon, err := c.db.GetArchivedConversation(conversationID)
		if err == nil && archivedCon != nil {
			if archivedCon.LatestMsgSendTime == msgCreateTime {
				return model_struct.LocalConversation{}, false
			}
		}
		//todo create a new conversation in local database
		newConv := model_struct.LocalConversation{}
		newConv.ConversationID = conversationID
		newConv.ConversationType = constant.OfficialArticlesChatType
		newConv.UserID = strconv.FormatInt(article.OfficialID, 10)
		newConv.ShowName = article.OfficialName
		newConv.FaceURL = article.OfficialProfileImg
		newConv.UnreadCount = 0
		newConv.UnreadNotificationCount = 1
		newConv.LatestMsg = string(jsonData)
		newConv.LatestMsgSendTime = msgCreateTime

		err = c.db.InsertConversation(&newConv)
		if err != nil {
			log.Debug(operationID, "New Article Conversation not inserted", newConv)
		}
		IsNewCreated = true
		return newConv, IsNewCreated
	} else {
		//Conversation exists use the same one and update it with the latest message and time and unread count
		if msgCreateTime != conversation.LatestMsgSendTime {
			conversation.UnreadNotificationCount = conversation.UnreadNotificationCount + 1
		}
		conversation.LatestMsg = string(jsonData)
		conversation.FaceURL = article.OfficialProfileImg
		conversation.ShowName = article.OfficialName
		conversation.LatestMsgSendTime = msgCreateTime
		err := c.db.UpdateConversation(conversation)
		if err != nil {
			log.Debug(operationID, "Pre Existed Article Conversation not updated", conversation)
		}
		return *conversation, IsNewCreated
	}
}
