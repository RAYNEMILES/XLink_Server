...
import (
	...
	common2 "Open_IM/cmd/Open-IM-SDK-Core/internal/common"
...
func (c *Conversation) doMsgNew(c2v common.Cmd2Value) {
	...
	needUpdateRevokeSeedTime := make(map[string]map[string]interface{})
	...
	for _, v := range allMsg {
		log.NewError(operationID, utils.GetSelfFuncName(), "do Msg come here, msg detail ", v.RecvID, v.SendID, v.ClientMsgID, v.ServerMsgID, v.Seq, c.loginUserID)
		...
		case constant.SingleChatType:
			...
			} else if v.ContentType == constant.GroupMemberSyncSilentSDKNotification {
				//todo do sync with local database
				log.Info(operationID, "DoGroupMsg SingleChatType SyncGroupMembers by Version", v)
				c.group.DoNotification(v, c.GetCh())
				continue
			} else if v.ContentType == constant.MomentNotification {
				//Update the trigger for notify for change in Moment
				log.Info(operationID, "Moment Change notification like Like, Comment, Reply etc", v)
				c.user.NotifyMomentNotification(v)
				continue
			} else if v.ContentType == constant.OfficialAccountFollowUnfollowNotification {
				//Update the trigger for notify for change in Moment
				log.Info(operationID, "Update Followed and Un followed Official Accounts", v)
				c.GetOfficialAccountConversationFormServer(operationID)
				continue
			}
		...
		if v.SendID == c.loginUserID { //seq // Messages sent by myself  //if  sent through  this terminal
			...
			if err == nil {
				...
				case constant.GroupChatType:
					...
					localGroup, _ := c.db.GetGroupInfoByGroupID(lc.GroupID)
					if localGroup != nil && localGroup.FaceURL != "" {
						lc.FaceURL = localGroup.FaceURL
					}
			...
			log.NewInfo(operationID, utils.GetSelfFuncName(), "Sent by others:", *msg)
			if _, err := c.db.GetMessageController(msg); err != nil { //Deduplication operation
				lc := model_struct.LocalConversation{
					ConversationType: v.SessionType,
				}
				lc.LatestMsg = utils.StructToJsonString(msg)
				lc.LatestMsgSendTime = msg.SendTime
				switch v.SessionType {
				case constant.SingleChatType:
				...
				if isUnreadCount {
					if msg.ContentType != constant.Revoke || msg.MsgFrom != 200 {
				...
				if isHistory {
					if msg.ContentType == constant.Revoke {
						revokeMessage := sdk_struct.MsgStruct{
							ClientMsgID: msg.Content,
							SessionType: msg.SessionType,
							GroupID:     msg.GroupID,
						}
						message, err := c.db.GetMessageController(&revokeMessage)

						if err == nil {
							msg.SendTime = message.SendTime
							msg.SortTime = message.SendTime
						} else {
							needUpdateRevokeSeedTime[msg.ClientMsgID] = map[string]interface{}{
								"GroupID":     msg.GroupID,
								"ClientMsgID": msg.Content,
								"SessionType": msg.SessionType,
							}
	...
	err2 := c.db.BatchInsertExceptionMsg(exceptionMsg)
	if err2 != nil {
	...
	if len(needUpdateRevokeSeedTime) > 0 {
		for clientMessageId, item := range needUpdateRevokeSeedTime {
			beRevokeMessage := sdk_struct.MsgStruct{
				ClientMsgID: item["ClientMsgID"].(string),
				SessionType: item["SessionType"].(int32),
				GroupID:     item["GroupID"].(string),
			}
			message, err := c.db.GetMessageController(&beRevokeMessage)
			if err == nil {
				c.db.UpdateColumnsMessageController(clientMessageId, beRevokeMessage.GroupID, beRevokeMessage.SessionType, map[string]interface{}{"send_time": strconv.FormatInt(message.SendTime, 10), "sort_time": strconv.FormatInt(message.SendTime, 10)})
			}
		}
	}
...
func (c *Conversation) doSuperGroupMsgNew(c2v common.Cmd2Value) {
	...
	for _, v := range allMsg {
		...
		msg := new(sdk_struct.MsgStruct)
		copier.Copy(msg, v)
		log.NewError(operationID, utils.GetSelfFuncName(), "Msg Recive Opition is Blocked 0")
		if v.SessionType == constant.GroupChatType {

			conversation, _ := c.db.GetConversation(utils.GetConversationIDBySessionType(v.GroupID, constant.GroupChatType))
			log.NewError(operationID, utils.GetSelfFuncName(), "Msg Recive Opition is Blocked 1", conversation)
			if conversation != nil {
				if conversation.RecvMsgOpt == 1 {
					log.NewError(operationID, utils.GetSelfFuncName(), "Msg Recive Opition is Blocked ", v)
					continue
				} else if conversation.RecvMsgOpt == 2 {
					isUnreadCount = false
				}
			}
		}
		...
		if v.SendID == c.loginUserID { //seq
			...
			m, err := c.db.GetMessageController(msg)
			if err == nil {
				log.NewError(operationID, "have message", msg.Seq, msg.ServerMsgID, msg.ClientMsgID, *msg)
				if m.Seq == 0 {
					if m.CreateTime == 0 {
				...
				log.NewError(operationID, "sync message", msg.Seq, msg.ServerMsgID, msg.ClientMsgID, *msg)
				lc := model_struct.LocalConversation{
					ConversationType:  v.SessionType,
					LatestMsg:         utils.StructToJsonString(msg),
					LatestMsgSendTime: msg.SendTime,
			...
			if oldMessage, err := c.db.GetMessageController(msg); err != nil { //Deduplication operation
				...
				case constant.GroupChatType:
					...
					log.Debug(operationID, "Friend user id: ", msg.SendID, "Current user id: ", c.loginUserID)
					friendInfoList, err := c.db.GetFriendInfoList([]string{msg.SendID})
					if err != nil {
						return
					}
					if len(friendInfoList) > 0 && friendInfoList[0].Remark != "" {
						msg.SenderNickname = friendInfoList[0].Remark
					}
				case constant.SuperGroupChatType:
					lc.GroupID = v.GroupID
					lc.ConversationID = utils.GetConversationIDBySessionType(lc.GroupID, constant.SuperGroupChatType)
	...
	if err1 != nil {
		for _, v := range insertMsg {
			log.Error(operationID, "insert GetMessage detail err:", err1.Error(), len(insertMsg), v.ClientMsgID, v.SessionType, v.RecvID)
			e := c.db.InsertMessageController(v)
			if e != nil {
				errChatLog := &model_struct.LocalErrChatLog{}
				copier.Copy(errChatLog, v)
	...
	for _, v := range exceptionMsg {
		log.Warn(operationID, "exceptionMsg show: ", *v)
		if v.RecvID != "" {
			err2 := c.db.SuperBatchInsertExceptionMsg([]*model_struct.LocalErrChatLog{v}, v.RecvID)
			if err2 != nil {
				log.Error(operationID, "insert err message err  :", err2.Error())
...
func (c *Conversation) revokeMessage(msgRevokeList []*sdk_struct.MsgStruct) {
	for _, w := range msgRevokeList {
		if c.msgListener != nil {
			...
			t.RecvID = w.GroupID
			//if w.MsgFrom == 200 {
			//	t.Status = constant.MsgStatusHasDeleted
			//	t.Seq = 0
			//	t.MsgFrom = 200
			//	t.IsRead = true
			//}
...
func (c *Conversation) DoGroupMsgReadStateOld(groupMsgReadList []*sdk_struct.MsgStruct) {
...
func (c *Conversation) DoGroupMsgReadState(msgReadList []*sdk_struct.MsgStruct) {
	var groupMessageReceiptResp []*sdk_struct.MessageReceipt
	...
	for _, rd := range msgReadList {
		...
		for _, v := range msgIdList {
			msg := sdk_struct.MsgStruct{ClientMsgID: v, SessionType: constant.GroupChatType, GroupID: rd.GroupID}
			m, err := c.db.GetMessageController(&msg)
			if err != nil {
			...
			m.IsRead = true
			err = c.db.UpdateMessageController(m)
			if err != nil {
		...
		conversationID = utils.GetConversationIDBySessionType(rd.GroupID, constant.GroupChatType)
	...
	if len(groupMessageReceiptResp) > 0 {
		...
		log.Info("internal", "OnRecvC2CReadReceipt: ", utils.StructToJsonString(groupMessageReceiptResp))
		c.msgListener.OnRecvGroupReadReceipt(utils.StructToJsonString(groupMessageReceiptResp))
	}
}
...
func (c *Conversation) doUpdateConversation(c2v common.Cmd2Value) {
	...
	case constant.AddConOrUpLatMsg:
		...
		if err == nil {
			...
			log.Info("this is new conversation UnreadCount and LatestMsgSendTime", lc.UnreadCount, lc.LatestMsgSendTime)
			if lc.LatestMsgSendTime >= oc.LatestMsgSendTime { //The session update of asynchronous messages is subject to the latest sending time
				...
				var OldConvLatestMsg sdk_struct.MsgStruct
				utils.JsonStringToStruct(lc.LatestMsg, &latestMsg)
				utils.JsonStringToStruct(oc.LatestMsg, &OldConvLatestMsg)
				if latestMsg.ContentType == constant.Revoke {
					if OldConvLatestMsg.ClientMsgID == latestMsg.Content {
						oc.LatestMsg = ""
					}
				...
				err := c.db.UpdateColumnsConversation(node.ConID, map[string]interface{}{"latest_msg_send_time": oc.LatestMsgSendTime, "latest_msg": oc.LatestMsg})
				if err != nil {
	...
	case constant.NotificationCountSetZero:
		if err := c.db.UpdateColumnsConversation(node.ConID, map[string]interface{}{"unread_notifications_count": 0}); err != nil {
			log.Error("internal", "UpdateColumnsConversation err", err.Error(), node.ConID)
		} else {
			totalUnreadCount, err := c.db.GetTotalUnreadMsgCount()
			if err == nil {
	...
	case constant.ConChange:
		...
		if err != nil {
			...
			log.Error("Sandman-8-1", "Con Change, Con List", cidList)
		} else {
			var newCList []*model_struct.LocalConversation
			for _, v := range cLists {
				if v.LatestMsgSendTime != 0 {
					newCList = append(newCList, v)
					log.Debug("Sandman-8-2", "Location Conv", v)
				}
				log.Debug("Sandman-8-3", "Location Conv", v)
			}
	...
	case constant.ConChangeDirect:
		cidList := node.Args.(string)
		log.Debug("", utils.GetSelfFuncName(), "constant.ConChangeDirect:", cidList)
		c.ConversationListener.OnConversationChanged(cidList)

	case constant.NewConDirect:
		cidList := node.Args.(string)
		log.Debug("", utils.GetSelfFuncName(), "constant.NewConDirect:", cidList)
...
func (c *Conversation) Work(c2v common.Cmd2Value) {
	...
	log.NewError("internal", "doListener work..", c2v.Cmd)
	...
	case constant.CmdNewMsgCome:
		log.NewError("internal", "doMsgNew start..", c2v.Cmd)
		c.doMsgNew(c2v)
		log.Info("internal", "doMsgNew end..", c2v.Cmd)
	case constant.CmdSuperGroupMsgCome:
		log.NewError("internal", "doSuperGroupMsgNew start..", c2v.Cmd)
	...
	if msg.ContentType >= constant.NotificationBegin && msg.ContentType <= constant.NotificationEnd {
		...
		case constant.AtText:
			...
			if err == nil {
				...
				if utils.IsContain(constant.AtAllString, msg.AtElem.AtUserList) {
					msg.AtElem.IsAtSelf = true
				}
			}
		...
		case constant.Article:
			err = utils.JsonStringToStruct(msg.Content, &msg.ArticleElem)
		case constant.WalletTransfer:
			err = utils.JsonStringToStruct(msg.Content, &msg.WalletTransferElem)
		case constant.OfficialAccount:
			err = utils.JsonStringToStruct(msg.Content, &msg.OfficialAccountElem)
		case constant.Calling:
			err = utils.JsonStringToStruct(msg.Content, &msg.CallingElem)
		case constant.Woom:
			err = utils.JsonStringToStruct(msg.Content, &msg.WoomElem)
		}
	}
...
func (c *Conversation) updateConversation(lc *model_struct.LocalConversation, cs map[string]*model_struct.LocalConversation) {
	if oldC, ok := cs[lc.ConversationID]; !ok {
		latestMsgStruct := sdk_struct.MsgStruct{}
		err := json.Unmarshal([]byte(lc.LatestMsg), &latestMsgStruct)
		if err == nil {
			if latestMsgStruct.ContentType >= constant.NotificationBegin && latestMsgStruct.ContentType <= constant.NotificationEnd {
				// set latest message empty, by GORM the latest message will not be replaced with this empty one.
				lc.LatestMsg = ""
			} else if latestMsgStruct.ContentType == constant.Revoke || latestMsgStruct.MsgFrom == 200 {
				dbConver, err := c.db.GetConversation(lc.ConversationID)
				if err == nil && dbConver != nil {
					dbConvLastMsgStruct := sdk_struct.MsgStruct{}
					err := json.Unmarshal([]byte(dbConver.LatestMsg), &dbConvLastMsgStruct)
					if err == nil {
						if dbConvLastMsgStruct.ClientMsgID == latestMsgStruct.Content {
							latestMsgStruct.ContentType = constant.Text
							latestMsgStruct.Content = ""
							lc.LatestMsg = utils.StructToJsonString(latestMsgStruct)
						} else {
							lc.LatestMsg = dbConver.LatestMsg
						}
					}
				}
		...
		err := json.Unmarshal([]byte(lc.LatestMsg), &latestMsgStruct)
		if err == nil {
			if latestMsgStruct.ContentType < constant.NotificationBegin {
...
func (c *Conversation) addFaceURLAndName(lc *model_struct.LocalConversation) {
	...
	case constant.SingleChatType, constant.NotificationChatType:
		faceUrl, name, accountStatus, err, isFromSvr := c.friend.GetUserNameAndFaceUrlByUid(lc.UserID, operationID)
		if err != nil {
		...
		lc.FaceURL = faceUrl
		lc.ShowName = name
		lc.AccountStatus = accountStatus
		if isFromSvr {
...
func (f *Conversation) processGetUserIPandStatus(callback open_im_sdk_callback.Base, userIPandStatusReq server_api_params.GetUserIPAndStatusReq, handleResult int32, operationID string) server_api_params.GetUserIPAndStatusResp {
	log.NewInfo(operationID, utils.GetSelfFuncName(), "args: ", userIPandStatusReq, handleResult)
	log.Error("Sandman & 4")
	var response = server_api_params.GetUserIPAndStatusResp{}
	//userIPandStatusReq.UserID = f.loginUserID
	//userIPandStatusReq.OperationID = operationID
	//userIPandStatusReq.IPAddress = f.WsConn.Conn.RemoteAddr().String()
	//_ = f.p.PostReturn(constant.GetUserIPandStatusRouter, userIPandStatusReq, &response)
