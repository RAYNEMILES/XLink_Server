...
func (g *Group) GetGroupMemberList(callback open_im_sdk_callback.Base, groupID string, filter, offset, count int32, nameSearch string, operationID string) {
	if callback == nil {
		return
	}
	fName := utils.GetSelfFuncName()
	go func() {
		...
		groupMemberList := g.getGroupMemberList(callback, groupID, filter, offset, count, nameSearch, operationID)
...
//func (g *Group) SyncGroupUpdatesVerionByGroupID(callback open_im_sdk_callback.Base, groupID string, operationID string) {
//	if callback == nil {
//		return
//	}
//	fName := utils.GetSelfFuncName()
//	go func() {
//		log.NewInfo(operationID, fName, "args: ", groupID)
//		g.syncGroupUpdatesVerionByGroupID(groupID, operationID)
//		callback.OnSuccess(utils.StructToJsonStringDefault("[]"))
//		log.NewInfo(operationID, fName, " callback: ", utils.StructToJsonStringDefault("[]"))
//	}()
//}

func (g *Group) SyncGroupUpdatesVersionByGroupIDV2(callback open_im_sdk_callback.Base, groupID string, operationID string) {
	if callback == nil {
		return
	}
	fName := utils.GetSelfFuncName()
...
func (g *Group) GetGroupMembersSyncStatus(callback open_im_sdk_callback.Base, groupID string, operationID string) {
	if callback == nil {
		return
	}
	fName := utils.GetSelfFuncName()
	...
	groupMemberSyncStatusModel, ok := g.groupMemberSyncStatus[groupID]
	if ok {
		response := make(map[string]interface{})
		response["syncInProgress"] = true
		response["groupID"] = groupID
		response["groupMemberSyncStatusModel"] = groupMemberSyncStatusModel
		callback.OnSuccess(utils.StructToJsonStringDefault(response))
	} else {
		response := make(map[string]interface{})
		response["syncInProgress"] = false
		response["groupID"] = groupID
		response["groupMemberSyncStatusModel"] = nil
		callback.OnSuccess(utils.StructToJsonStringDefault(response))
	}

}

func (g *Group) GetFriendListNotMemberOfTheGroup(callback open_im_sdk_callback.Base, groupID string, operationID string) {
	if callback == nil {
		return
	}
	fName := utils.GetSelfFuncName()
	log.NewInfo(operationID, fName, "args: ", groupID)
	localFriendList := g.getFriendListNotMemberOfTheGroup(groupID, operationID)
	callback.OnSuccess(utils.StructToJsonStringDefault(localFriendList))
