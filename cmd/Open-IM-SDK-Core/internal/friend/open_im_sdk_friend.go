...
func (f *Friend) GetFriendApplicationList(callback open_im_sdk_callback.Base, operationID string) {
	if callback == nil {
		log.Error(operationID, "callback is nil")
		return
	}
	...
	go func() {
		log.NewInfo(operationID, fName, "args: ")
		result := f.getFriendApplicationList(callback, operationID)
		callback.OnSuccess(utils.StructToJsonStringDefault(result))
		log.NewInfo(operationID, fName, " callback: ", utils.StructToJsonStringDefault(result))
	}()
}
...
func (f *Friend) DeleteFriend(callback open_im_sdk_callback.Base, friendUserID string, operationID string) {
	...
	go func() {
		...
		callback.OnSuccess(friendUserID)
...
func (f *Friend) GetFriendRemarkOrNick(callback open_im_sdk_callback.Base, userIDGroupID string, operationID string) {
	if callback == nil {
		log.Error(operationID, "callback is nil")
		return
	}
	fName := utils.GetSelfFuncName()
	go func() {
		log.NewInfo(operationID, fName, "args: ", userIDGroupID)
		var unmarshalParams sdk.GetFriendRemarkOrNickParams
		common.JsonUnmarshalAndArgsValidate(userIDGroupID, &unmarshalParams, callback, operationID)
		result, _ := f.getFriendRemarkOrNick(unmarshalParams, callback, operationID)
		// callback.OnSuccess(utils.StructToJsonString(sdk.SetFriendRemarkCallback))
...
func (f *Friend) GetUserRemarkLocal(callback open_im_sdk_callback.Base, userID string, operationID string) {
	if callback == nil {
		log.Error(operationID, "callback is nil")
		return
	}
	fName := utils.GetSelfFuncName()
	go func() {
		localFriendRemark := sdk.LocalFriendRemark{}
		localFriendRemark.FriendUserID = userID

		log.NewInfo(operationID, fName, "args: ", userID)
		localFriend, _ := f.db.GetFriendInfoByFriendUserID(userID)
		if localFriend != nil {
			localFriendRemark.Remark = localFriend.Remark
		}
		callback.OnSuccess(utils.StructToJsonString(localFriendRemark))
		log.NewInfo(operationID, fName, " callback: ", utils.StructToJsonString(localFriendRemark))
	}()
}
