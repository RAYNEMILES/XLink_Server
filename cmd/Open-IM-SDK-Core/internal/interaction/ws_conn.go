...
type WsConn struct {
	...
	platformID  int32
	Conn        *websocket.Conn
	syncCh      chan common.Cmd2Value
}

func NewWsConn(listener open_im_sdk_callback.OnConnListener, token string, loginUserID string, platformID int32, syncCh chan common.Cmd2Value) *WsConn {
	p := WsConn{listener: listener, token: token, loginUserID: loginUserID, platformID: platformID, syncCh: syncCh}
	//go func() {
	var err error
	p.conn, err, _ = p.ReConn()
	p.Conn = p.conn
	if err != nil {
		log.Error("", utils.GetSelfFuncName(), "NewWsConn failed:", err.Error())
	}
	//}()
	return &p
}
...
func (u *WsConn) SendPingMsg() error {
	...
	if u.conn == nil {
		log.Warn("", utils.GetSelfFuncName(), "conn == nil")
		//u.ReConn()
	...
	if err != nil {
		u.loginState = constant.Disconnected
		log.Warn("", utils.GetSelfFuncName(), "SetWriteTimeout", err.Error())
		return utils.Wrap(err, "SetWriteDeadline failed")
	}
	err = u.conn.WriteMessage(websocket.PingMessage, []byte(ping))
	if err != nil {
		u.loginState = constant.Disconnected
		log.Warn("", utils.GetSelfFuncName(), "WriteMessage", err.Error())
		return utils.Wrap(err, "WriteMessage failed")
	}
	//u.loginState = constant.LoginSuccess
	//log.NewError("", utils.GetSelfFuncName(), "ping success!", u.loginUserID)
	return nil
}
...
func (u *WsConn) writeBinaryMsg(msg GeneralWsReq) (*websocket.Conn, error) {
	...
	if u.conn != nil {
		err := u.SetWriteTimeout(writeTimeoutSeconds)
		if err != nil {
			u.SetLoginState(constant.Disconnected)
		...
		wsCon, err := u.conn, utils.Wrap(u.conn.WriteMessage(websocket.BinaryMessage, buff.Bytes()), "")
		if err != nil {
			u.SetLoginState(constant.Disconnected)
		}
		return wsCon, err
...
func (u *WsConn) ReConn() (*websocket.Conn, error, bool) {
	...
	url := fmt.Sprintf("%s?sendID=%s&token=%s&platformID=%d&operationID=%s", sdk_struct.SvrConf.WsAddr, u.loginUserID, u.token, u.platformID, operationID)
	log.Debug("", utils.GetSelfFuncName(), "connect to msg_gateway url:", url)
	...
	if err != nil {
		log.NewError(operationID, "ws connect failed ", url, err.Error())

		u.loginState = constant.LoginFailed
		if httpResp != nil {
			errMsg := httpResp.Header.Get("ws_err_msg") + " operationID " + operationID + err.Error()
			log.NewError(operationID, "websocket.DefaultDialer.Dial failed ", errMsg, httpResp.StatusCode)
			...
			errMsg := err.Error() + " operationID " + operationID
			u.listener.OnConnectFailed(1001, errMsg)
			log.NewError(operationID, "websocket.DefaultDialer.Dial failed ", errMsg, "url ", url)
			return nil, utils.Wrap(err, errMsg), true
		}
	}
	...
	u.conn = conn
	u.conn.SetPongHandler(func(pongMsg string) error {
		u.conn.SetReadDeadline(time.Now().Add(time.Second * 8))
		return nil
	})
	u.conn.SetCloseHandler(func(code int, text string) error {
		log.Debug("Sandman Napster", "WebSocket status : ", code)
		u.SetLoginState(constant.Disconnected)
	...
	//log.NewError(operationID, utils.GetSelfFuncName(), "conn:", u.loginUserID, conn, conn.RemoteAddr())
