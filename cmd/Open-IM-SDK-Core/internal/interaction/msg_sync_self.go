...
type SelfMsgSync struct {
	...
	*Ws
	...
	pushMsgCache       map[uint32]*sdk.MsgData
}
...
func NewSelfMsgSync(dataBase *db.DataBase, ws *Ws, loginUserID string, conversationCh chan common.Cmd2Value) *SelfMsgSync {
	...
	p.pushMsgCache = make(map[uint32]*sdk.MsgData, 0)
	return p
}
...
func (m *SelfMsgSync) doMaxSeq(cmd common.Cmd2Value) {
	...
	//log.NewError(operationID, utils.GetSelfFuncName(), "start!!")
...
func (m *SelfMsgSync) doPushBatchMsg(cmd common.Cmd2Value) {
	...
	if len(msgDataWrap.MsgDataList) == 1 && msgDataWrap.MsgDataList[0].Seq == 0 {
		...
		m.TriggerCmdNewMsgCome([]*sdk.MsgData{msgDataWrap.MsgDataList[0]}, operationID)
		return
	}
	...
	var triggerMsgList []*sdk.MsgData
	for {
...
func (m *SelfMsgSync) doPushSingleMsg(cmd common.Cmd2Value) {
	msg := cmd.Value.(sdk_struct.CmdPushMsgToMsgSync).Msg
	operationID := cmd.Value.(sdk_struct.CmdPushMsgToMsgSync).OperationID
	//log.NewError(operationID, utils.GetSelfFuncName(), " args: ", msg.Seq, msg.ServerMsgID, msg.ClientMsgID, m.seqMaxNeedSync, m.seqMaxSynchronized)
	if msg.Seq == 0 {
		log.Debug(operationID, "TriggerCmdNewMsgCome ", msg.ServerMsgID, msg.ClientMsgID, msg.Seq)
		m.TriggerCmdNewMsgCome([]*sdk.MsgData{msg}, operationID)
		return
	}

	//seq连续，则消息直接触发
	//log.NewError(operationID, utils.GetSelfFuncName(), "msg---Seq", m.loginUserID, msg.ServerMsgID, msg.ClientMsgID, msg.Seq, m.seqMaxSynchronized, m.seqMaxNeedSync)
	if msg.Seq == m.seqMaxSynchronized+1 {
		//log.NewError(operationID, "msg.Seq == m.seqMaxSynchronized+1", msg.ServerMsgID, msg.ClientMsgID, msg.Seq)
		m.TriggerCmdNewMsgCome([]*sdk.MsgData{msg}, operationID)
		m.seqMaxSynchronized = msg.Seq
	}
	if msg.Seq > m.seqMaxNeedSync {
		//log.NewError(operationID, "msg.Seq > m.seqMaxNeedSync", msg.ServerMsgID, msg.ClientMsgID, msg.Seq)
...
func (m *SelfMsgSync) syncMsg(operationID string) {
	if m.seqMaxNeedSync > m.seqMaxSynchronized {
		//log.NewError(operationID, "do syncMsgFromServer ", m.seqMaxSynchronized+1, m.seqMaxNeedSync)
...
func (m *SelfMsgSync) syncMsgFromServer(beginSeq, endSeq uint32, operationID string) {
	//log.NewError(operationID, utils.GetSelfFuncName(), " args ", beginSeq, endSeq)
	if beginSeq > endSeq {
...
//触发新消息
func (m *SelfMsgSync) TriggerCmdNewMsgCome(msgList []*sdk.MsgData, operationID string) {
	for {
		...
		//log.NewError(operationID, "TriggerCmdNewMsgCome ok ", m.loginUserID)
		return
	}
}
